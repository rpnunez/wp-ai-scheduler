...
## 2024-05-26 - [Extract Prompt Builder] **Context:** `AIPS_Generator` was violating SRP by mixing orchestration logic with the specific string construction of AI prompts (system instructions, formatting rules, voice application). **Decision:** Extracted prompt construction logic into a new `AIPS_Prompt_Builder` class. Refactored `AIPS_Generator` to delegate prompt building to this new service. **Consequence:** `AIPS_Generator` is now focused purely on orchestration; Prompt logic is isolated and easier to test/modify; Increased file count by 1.
