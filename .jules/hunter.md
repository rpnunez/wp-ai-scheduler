...
## 2024-10-24 - Prevent TypeError on nested array input in Planner
**Learning:** `$_POST['topics']` is expected to be an array of strings, but an attacker could supply an array of arrays. Passing this nested array to `array_map('sanitize_text_field', ...)` causes a fatal `TypeError` in PHP 8+ since `sanitize_text_field()` strictly requires a string.
**Action:** When sanitizing arrays from user input that are expected to contain strings, iterate over them (e.g., using `foreach`) and explicitly check `is_scalar()` before passing them to the sanitization function, instead of blindly using `array_map()`.
