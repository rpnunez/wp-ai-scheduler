...
## 2024-05-26 - Missing URL escaping in JSON responses
**Learning:** URLs generated by WordPress functions like `get_permalink()` or `get_edit_post_link()` can be a vector for Javascript injection if returned directly in an API or AJAX response (e.g., `wp_send_json_success`) and then output to the DOM by the frontend client.
**Action:** Always wrap dynamically generated URLs with `esc_url_raw()` before sending them in JSON responses.
