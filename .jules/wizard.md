...
## $(date +%Y-%m-%d) - Post Preview Added to Generated Posts and Partial Generations Tabs
...
Action: Reuse the `.aips-preview-post` class and `aips_get_draft_post_preview` AJAX action from `admin-post-review.js` by simply adding the preview button to the other tabs in the Content view.
