...
## 2024-05-18 - Fix Array Type Validation and Magic Quotes Bypass
**Vulnerability:** Superglobal arrays `$_POST['slice_ids']` in `class-aips-post-slices-controller.php` were retrieved using `(array)` casting instead of proper type checking (`is_array()`) and failed to invoke `wp_unslash()` before sanitization, introducing a potential type coercion flaw and magic quotes bypass.
**Learning:** Developers sometimes use `(array)` to force string inputs into arrays or omit `is_array` checks because functions like `array_map` throw a TypeError otherwise, while `wp_unslash` is often forgotten for bulk operations.
**Prevention:** Always validate that incoming superglobal parameters intended to be arrays genuinely pass `is_array()` checks. Apply `wp_unslash()` to the entire array before running individual item sanitizations (e.g. `absint` or `sanitize_text_field`).
