...
## 2024-05-04 - Stop Leaking Internal WP_Error Details in AJAX Endpoints
**Vulnerability:** Numerous AJAX endpoints were directly returning internal `WP_Error` messages (via `get_error_message()`) to the client without logging them first, leaking internal mechanics to the end user.
**Learning:** Returning explicit API error messages or internal system failures directly to the client is a security risk. This likely happened for convenience during development to surface errors easily.
**Prevention:** Always log specific technical errors internally (e.g. `error_log`) and respond to the client with generic, non-revealing error messages using localized strings.
