...
## 2024-05-18 - Fix PHP 8 TypeError DoS in array processing
**Vulnerability:** Unsafe array processing from user input (`$_POST`) was passed directly to `array_map('sanitize_text_field', ...)` in multiple files (`class-aips-structures-controller.php`, `class-aips-research-controller.php`, `class-aips-planner.php`). In PHP 8+, passing a nested array (which can be intentionally submitted by an attacker) to a string-expecting sanitization function via `array_map` results in an uncaught `TypeError`, causing a Denial of Service (DoS) fatal error.
**Learning:** Blindly mapping string-based sanitization functions over raw arrays from superglobals without validating that the array elements are scalar values creates a vulnerability.
**Prevention:** Iterate through user-provided arrays using a `foreach` loop and explicitly verify each item with `is_scalar()` before passing it to sanitization functions and casting it to a string.
