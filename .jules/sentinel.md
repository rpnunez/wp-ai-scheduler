...
## 2025-05-24 - [CRITICAL] Fix Complete Data Wipes in History Deletion
**Vulnerability:** Found a logic flaw/vulnerability in `delete_by_status()` and `clear_history()` inside `AIPS_History_Repository` where empty filtering arguments implicitly lead to `DELETE FROM table` directly, causing complete data wipe.
**Learning:** This existed because the logic assumed empty input meant "do not filter", hence deleting everything, without explicitly confirming intention.
**Prevention:** Always require explicit intentions (e.g. `$status === 'all'`) before issuing table-wide delete commands, especially when arguments are passed dynamically.
