...
### 1) Scheduler wrapper methods with no external call sites found
- Methods:
...
### 2) Injection helpers currently unused by internal code/tests
- Methods:
...
### 3) Residual schedule-wizard wording/hash markers in admin.js (stale, not functional code)
...
1. Deprecate and then remove unused scheduler wrapper/injection methods after one release cycle.
2. Clean stale schedule-wizard comments/hash references in admin.js for consistency.
