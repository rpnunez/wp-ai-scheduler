...
- `[ ]` **Phase 4: Minor Admin Views & System Tools**
  - `[x]` Create `views/base-form.js` (Shared `BaseFormView` parent class)
  - `[ ]` Create `models/telemetry.js` & `views/telemetry.js` (Telemetry logs pagination, details payload, Chart.js charts)
  - `[ ]` Create `models/campaign.js` & `views/campaigns.js` (Campaign list and Campaigns Wizard)
  - `[ ]` Create `views/settings.js` (Settings tabs connection tests extending BaseFormView)
  - `[ ]` Create `views/post-slices.js` (Content slices manager extending BaseFormView)
  - `[ ]` Create `views/system-status.js` (Diagnostics checklist and copy status)
  - `[ ]` Create `views/onboarding.js` (Onboarding installation steps wizard)
  - `[ ]` Create `views/dev-tools.js` (Developer tools view, clear cache monitor, DB repair triggers, mock data seeding)
  - `[ ]` Create `views/embeddings.js` (Embeddings indexing progress bar)
  - `[ ]` Create `views/post-review.js` (Pending reviews & AI regeneration)
  - `[ ]` Create `views/admin-bar.js` & `views/block-editor.js` (WP toolbar hooks & inline block editor tools)
