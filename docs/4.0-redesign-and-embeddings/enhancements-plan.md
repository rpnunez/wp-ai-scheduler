...
| Tier 3: items 11–15 | ✅ Item 13 (T3-1). ✅ Redirects module (T3-6). ✅ Item 12's consolidation flow (T3-7). ✅ Item 11: silos (T3-2/T3-3). ⏸ Items 14 and 15 are on hold |
...
Order (updated 2026-09-24): 13 → 12 → 11. Items 13, 12 and 11 are done. Items 14 and 15 are on hold. The slices are defined in the plan, section 2.
...
| T3-2 | Silo model (11) | ✅ | `AIPS_Silo_Service`: silos are clusters with a confirmed pillar (`pillar_confirmed`). Per-article coverage covers links up to the pillar and pillar → article through text or the guide list, plus a health score. The pillar suggestion scores inbound links 40%, words 30% and topic match 30%. |
| T3-3 | Silo UI and Fix silo (11) | ✅ | Content → Silos. Fix Silo is an auto-link run limited to the silo's articles, follows Bulk Auto-Linking (D21), and can be undone from the Link Report. The render-time "In this guide" list, shortcode and settings follow D19–D20, and the link index counts the list. Smoke-tested: overview, confirm and change pillar, list options, shortcode, front-end render, fix plus undo, and cap exemption. |
...
| — | Code review pass (whole branch) | ✅ | High-effort review of the full diff against `refactor/admin-ia`. Nonces/capabilities and every new setting's plumbing checked clean. 7 findings fixed: publish-linking undo not clearing its "already linked" flag (+ auto re-trigger on next save), an N+1 counts query in auto-link runs, an N+1 in the broken-link fixer's suggestions, silo re-indexing on every cluster re-detection instead of only on real change, a non-atomic click-tracking rate limit, a subdirectory-install path mismatch in redirects, and a fragile `has_action()` check in the silo pillar-confirm flow. See rpnunez/wp-ai-scheduler#2128 for the write-up. |
| T3-7 | Consolidation flow (12) | ✅ | Consolidate button on post-vs-post Cannibalization Shield pairs. Keep one post; the other goes to draft, gets a 301 through the redirect manager (`origin = consolidation`), and its inbound links are re-pointed. Optional AI-merged draft, saved as a revision or written into the kept post. Posts Consolidated notification. Recent Consolidations card with undo. Smoke-tested in Docker for none, revision, rewrite, undo, undo after an edit, and validation. |
...
| D16 | Silo "Fix silo" direction | ✅ Option C: members → pillar links in the text; the pillar gets a render-time "In this guide" list of members, so its text is never edited. |
| D17 | Consolidation: retired post | ✅ Moved to draft, plus a "Posts Consolidated" notification (channels set in Settings → Notifications; DB + email by default). |
| D19 | Silo pillars | ✅ Only confirmed pillars form silos; AIPS suggests one for the other clusters, and nothing happens until you confirm. |
| D20 | "In this guide" list | ✅ End of the post by default, or after the first paragraph (setting). Cap 10, closest first, adjustable. Style: an AIPS box or plain theme HTML (setting). A shortcode places it by hand. |
| D21 | Fix silo policy | ✅ Follows Bulk Auto-Linking (insert confident links, review the rest); exempt from the per-target inbound cap. Silo health shows only on the Silos screen. |
| D18 | Consolidation: merged content | ✅ Optional AI-merged draft, reviewable and editable, then saved as a revision of the kept post or written into it. |
