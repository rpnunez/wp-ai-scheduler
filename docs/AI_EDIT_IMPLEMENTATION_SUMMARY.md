...
All planned features from the roadmap have been implemented, tested, and documented.
...
- Methods:
  - `get_generation_context($history_id)` - Retrieves original generation context
  - `regenerate_title($context)` - Regenerates post title
  - `regenerate_excerpt($context)` - Regenerates post excerpt
  - `regenerate_content($context)` - Regenerates post content (supports structures)
  - `regenerate_featured_image($context)` - Regenerates featured image
- Dependencies: Generator, Prompt Builder, Image Service, Repositories
...
- Endpoints:
  ...
  - `aips_regenerate_component` - Regenerates single component
