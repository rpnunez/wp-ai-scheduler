...
- **Observability baseline metrics**: scheduler and generation reliability metrics reviewable from System Status.
  - New `AIPS_Metrics_Repository` class — collects and exposes summary metrics from existing tables and post-meta (no schema changes); results are cached in WordPress transients (15-minute TTL, invalidate-able via `invalidate_cache()`).
