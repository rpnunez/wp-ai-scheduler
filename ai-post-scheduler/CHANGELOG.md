...
## [1.6.1] - 2025-12-25
### Fixed
- **Security:** Fixed potential XSS vulnerability in the "View Posts" modal by escaping URLs generated by `get_permalink` and `get_edit_post_link` in `AIPS_Templates_Controller`.
- **Stability:** Fixed boolean validation logic in `AIPS_Schedule_Controller` to correctly handle `is_active` status updates, preventing issues where status might not update correctly.

### Improved
- **Architecture:** Refactored `AIPS_Schedule_Controller` to remove direct database access fallback, ensuring all schedule updates go through the `AIPS_Scheduler` service method `toggle_active`.
- **Utility:** Verified accessibility labels for toggle switches in the Schedule interface.
