(function($) {
    ...
    Object.assign(AIPS, {
        ...
        filterSections: function() {
            ...
            $rows.each(function() {
                var $row = $(this);
                var name = $row.find('td:first-child').text().toLowerCase(); // Name is first column
                var key = $row.find('.column-key').text().toLowerCase(); // Key is second column

                if (name.indexOf(term) > -1 || key.indexOf(term) > -1) {
                    $row.show();
                    hasVisible = true;
                } else {
                    $row.hide();
