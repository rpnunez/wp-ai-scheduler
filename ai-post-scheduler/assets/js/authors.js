...
(function ($) {
	...
	const AuthorsModule = {
		...
		bindEvents: function () {
			...
			// Author Search
			$('#aips-author-search').on('keyup input', this.handleAuthorSearch.bind(this));
			$('#aips-author-search-clear').on('click', this.clearAuthorSearch.bind(this));
			$('.aips-clear-author-search-btn').on('click', this.clearAuthorSearch.bind(this));
		...
		handleAuthorSearch: function(e) {
			...
			$rows.each(function() {
				const $row = $(this);
				// Name is in the first column, Niche in the second
		...
		clearAuthorSearch: function(e) {
			e.preventDefault();
			$('#aips-author-search').val('').trigger('input');
			$('#aips-author-search').focus();
		},
		...
		viewTopicPosts: function (e) {
			...
			$('#aips-topic-posts-modal').data('topic-id', topicId);
			...
			$.ajax({
				...
				success: (response) => {
					if (response.success) {
						...
						this.renderTopicPosts(posts, topicId);
					} else {
		...
		renderTopicPosts: function (posts, topicId) {
			...
			posts.forEach(post => {
				...
				html += '<button class="button aips-btn-danger aips-delete-generated-post" data-post-id="' + post.post_id + '" data-topic-id="' + topicId + '">' + (aipsAuthorsL10n.delete || 'Delete') + '</button>';
		...
		regeneratePost: function(e) {
			...
			if (!confirm(aipsAuthorsL10n.confirmRegeneratePost || 'Are you sure you want to regenerate this post? This will overwrite the existing content.')) {
				return;
			}
		...
		deleteGeneratedPost: function(e) {
			...
			if (!confirm(aipsAuthorsL10n.confirmDeletePost || 'Are you sure you want to delete this post? This cannot be undone.')) {
				return;
			}
