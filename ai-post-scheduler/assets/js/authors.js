...
(function ($) {
	...
	const AuthorsModule = {
		...
		bindEvents: function () {
			...
			// Search Topics
			$('#aips-topic-search').on('input', this.searchTopics.bind(this));
		...
		saveAuthor: function (e) {
			...
			$submitBtn.prop('disabled', true).addClass('aips-loading').text(aipsAuthorsL10n.saving);
			...
			$.ajax({
				...
				complete: () => {
					$submitBtn.prop('disabled', false).removeClass('aips-loading').text(aipsAuthorsL10n.saveAuthor);
				}
			});
		},
		...
		deleteAuthor: function (e) {
			...
			// Add loading state
			$btn.prop('disabled', true).addClass('aips-loading');
			...
			$.ajax({
				...
				success: (response) => {
					if (response.success) {
						...
						$btn.prop('disabled', false).removeClass('aips-loading');
					}
				},
				error: () => {
		...
		updatePagination: function (pagination) {
			if (!pagination || pagination.total === 0) {
				$('#aips-topics-pagination').hide();
				return;
			}

			$('#aips-current-page').text(pagination.current_page);
			$('#aips-total-pages').text(pagination.pages);
			$('.aips-pagination-prev').prop('disabled', pagination.current_page <= 1);
			$('.aips-pagination-next').prop('disabled', pagination.current_page >= pagination.pages);
			$('#aips-topics-pagination').css('display', 'flex');
		},
		...
		switchTab: function (e) {
			...
			// Hide search bar if feedback tab is active
			if (status === 'feedback') {
				$('#aips-topic-search').hide();
			} else {
				$('#aips-topic-search').show();
			}
		...
		loadFeedback: function () {
			...
			// Hide pagination for feedback for now as it wasn't implemented in backend
			$('#aips-topics-pagination').hide();
