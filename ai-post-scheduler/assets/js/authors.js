...
(function ($) {
	...
	const AuthorsModule = {
		...
		bindEvents: function () {
			...
			// View author posts
			$(document).on('click', '.aips-view-author-posts', this.viewAuthorPosts.bind(this));

			// Post actions
			$(document).on('click', '.aips-regenerate-post', this.regeneratePost.bind(this));
			$(document).on('click', '.aips-delete-generated-post', this.deleteGeneratedPost.bind(this));
		...
		viewAuthorPosts: function (e) {
			e.preventDefault();
			const authorId = $(e.currentTarget).data('id');
			this.currentAuthorId = authorId;

			$('#aips-author-posts-content').html('<p>' + (aipsAuthorsL10n.loadingPosts || 'Loading posts...') + '</p>');
			$('#aips-author-posts-modal').fadeIn();

			this.loadAuthorPosts(authorId);
		},

		loadAuthorPosts: function (authorId) {
			$.ajax({
				url: ajaxurl,
				type: 'POST',
				data: {
					action: 'aips_get_author_posts',
					nonce: aipsAuthorsL10n.nonce,
					author_id: authorId
				},
				success: (response) => {
					if (response.success) {
						this.renderAuthorPosts(response.data.posts);
					} else {
						$('#aips-author-posts-content').html(
							'<p>' + (response.data && response.data.message ? response.data.message : (aipsAuthorsL10n.errorLoadingPosts || 'Error loading posts')) + '</p>'
						);
					}
				},
				error: () => {
					$('#aips-author-posts-content').html('<p>' + (aipsAuthorsL10n.errorLoadingPosts || 'Error loading posts') + '</p>');
				}
			});
		},

		renderAuthorPosts: function (posts) {
			if (!posts || posts.length === 0) {
				$('#aips-author-posts-content').html('<p>' + (aipsAuthorsL10n.noPostsFound || 'No posts found.') + '</p>');
				return;
			}

			let html = '<table class="wp-list-table widefat fixed striped"><thead><tr>';
			html += '<th>' + (aipsAuthorsL10n.postId || 'ID') + '</th>';
			html += '<th>' + (aipsAuthorsL10n.topic || 'Topic') + '</th>';
			html += '<th>' + (aipsAuthorsL10n.postTitle || 'Post Title') + '</th>';
			html += '<th>' + (aipsAuthorsL10n.dateGenerated || 'Generated') + '</th>';
			html += '<th>' + (aipsAuthorsL10n.datePublished || 'Published') + '</th>';
			html += '<th>' + (aipsAuthorsL10n.actions || 'Actions') + '</th>';
			html += '</tr></thead><tbody>';

			posts.forEach(post => {
				html += '<tr>';
				html += '<td>' + this.escapeHtml(post.post_id) + '</td>';
				html += '<td>' + this.escapeHtml(post.topic_title || '-') + '</td>';
				html += '<td>' + this.escapeHtml(post.post_title) + '</td>';
				html += '<td>' + this.escapeHtml(post.created_at || post.date_generated || '') + '</td>';
				html += '<td>' + this.escapeHtml(post.date_published || aipsAuthorsL10n.notPublished || 'Not Published') + '</td>';
				html += '<td>';
				if (post.edit_url) {
					html += '<a href="' + this.sanitizeUrl(post.edit_url) + '" class="button" target="_blank">' + (aipsAuthorsL10n.editPost || 'Edit') + '</a> ';
				}
				if (post.post_url && post.post_status === 'publish') {
					html += '<a href="' + this.sanitizeUrl(post.post_url) + '" class="button" target="_blank">' + (aipsAuthorsL10n.viewPost || 'View') + '</a> ';
				}

				const topicId = post.author_topic_id;

				html += '<button class="button aips-regenerate-post" data-post-id="' + post.post_id + '" data-topic-id="' + topicId + '">' + (aipsAuthorsL10n.regenerate || 'Regenerate') + '</button> ';
				html += '<button class="button aips-delete-generated-post" data-post-id="' + post.post_id + '" style="color: #d63638;">' + (aipsAuthorsL10n.delete || 'Delete') + '</button>';
			...
			html += '</tbody></table>';
			$('#aips-author-posts-content').html(html);
		},
		...
		regeneratePost: function(e) {
			...
			if (!confirm(aipsAuthorsL10n.confirmRegeneratePost || 'Are you sure you want to regenerate this post? The current content will be overwritten.')) {
				return;
			}

			const $btn = $(e.currentTarget);
			$btn.prop('disabled', true).text(aipsAuthorsL10n.generating || 'Generating...');
			...
			$.ajax({
				...
				success: (response) => {
					if (response.success) {
						showToast(response.data.message || (aipsAuthorsL10n.postRegenerated || 'Post regenerated successfully.'), 'success');

						if ($('#aips-author-posts-modal').is(':visible') && this.currentAuthorId) {
							this.loadAuthorPosts(this.currentAuthorId);
						}

						if ($('#aips-topic-posts-modal').is(':visible')) {
							this.loadTopicPosts(topicId);
						}
					} else {
						showToast(response.data && response.data.message ? response.data.message : (aipsAuthorsL10n.errorRegenerating || 'Error regenerating post'), 'error');
						$btn.prop('disabled', false).text(aipsAuthorsL10n.regenerate || 'Regenerate');
					}
				},
				error: () => {
					showToast(aipsAuthorsL10n.errorRegenerating || 'Error regenerating post', 'error');
					$btn.prop('disabled', false).text(aipsAuthorsL10n.regenerate || 'Regenerate');
				}
			});
		},

		deleteGeneratedPost: function(e) {
			e.preventDefault();
			const postId = $(e.currentTarget).data('post-id');

			if (!confirm(aipsAuthorsL10n.confirmDeletePost || 'Are you sure you want to delete this post? This action cannot be undone.')) {
				return;
			}

			const $btn = $(e.currentTarget);
			$btn.prop('disabled', true).text(aipsAuthorsL10n.deleting || 'Deleting...');

			$.ajax({
				url: ajaxurl,
				type: 'POST',
				data: {
					action: 'aips_delete_generated_post',
					nonce: aipsAuthorsL10n.nonce,
					post_id: postId
				},
				success: (response) => {
					if (response.success) {
						showToast(response.data.message || (aipsAuthorsL10n.postDeleted || 'Post deleted successfully.'), 'success');
						...
						showToast(response.data && response.data.message ? response.data.message : (aipsAuthorsL10n.errorDeletingPost || 'Error deleting post'), 'error');
						$btn.prop('disabled', false).text(aipsAuthorsL10n.delete || 'Delete');
					}
				},
				error: () => {
					showToast(aipsAuthorsL10n.errorDeletingPost || 'Error deleting post', 'error');
					$btn.prop('disabled', false).text(aipsAuthorsL10n.delete || 'Delete');
				}
			});
		},
			...
			$.ajax({
				...
				success: (response) => {
					if (response.success) {
						...
						this.renderTopicPosts(posts, topic);
					} else {
		...
		renderTopicPosts: function (posts, topic) {
			if (!posts || posts.length === 0) {
