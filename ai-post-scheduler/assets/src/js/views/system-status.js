...
export const SystemStatusView = Backbone.View.extend({
	...
	initialize() {
		...
		if (this.isSystemStatusPage()) {
			this.syncStatusSectionsToggleState();
		}
	},

	isSystemStatusPage() {
		return this.$('.aips-status-data-panel').length > 0;
	},

	/**
	 * Toggle log detail table rows
	 */
	toggleLogDetails(e) {
		e.preventDefault();

		const $link = $(e.currentTarget);
		const target = $link.attr('data-target') || $link.data('target');
	...
	/**
	 * Toggle status section when clicking on panel header area (except interactive nodes)
	 */
	...
	/**
	 * Synchronize "Expand all / Collapse all" state
	 */
	syncStatusSectionsToggleState() {
		...
		if (hiddenCount === 0) {
			$toggle.attr('data-mode', 'collapse').text(this.l10n.collapseAll || 'Collapse all');
		} else {
			$toggle.attr('data-mode', 'expand').text(this.l10n.expandAll || 'Expand all');
		}
	},

	/**
	 * Toggle single section collapse state
	 */
	toggleStatusSection(e) {
		...
		const targetId = $button.attr('data-target') || $button.data('target');
		...
		$label.text(nextExpanded ? (this.l10n.collapseLabel || 'Collapse') : (this.l10n.expandLabel || 'Expand'));
	...
	toggleAllStatusSections(e) {
		...
		if (shouldExpand) {
			...
			$sectionButtons.find('.aips-panel-collapse-label').text(this.l10n.collapseLabel || 'Collapse');
		} else {
			...
			$sectionButtons.find('.aips-panel-collapse-label').text(this.l10n.expandLabel || 'Expand');
		}
	...
	/**
	 * Run a manual diagnostics/cleanup operation
	 */
	runStatusOperation(e) {
		e.preventDefault();
		const $btn = $(e.currentTarget);
		const action = $btn.attr('data-op') || $btn.data('op');
