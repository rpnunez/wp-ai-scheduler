...
class Config {
	...
	/**
	 * @var Cache Per-request cache for get_option() calls.
	 */
	private $cache = null;

	/**
	 * @var object Sentinel stored in the cache when the resolved option
	 *             value is null. Required because Cache::has() uses
	...
	private function __construct() {
		$this->cache         = CacheFactory::named('aips_config', 'array');
	...
	public function get_default_options() {
		return array(
			...
			// Site content strategy defaults (must match Settings::get_content_strategy_options()).
			'aips_site_niche' => '',
	 ...
	 * differ from the authoritative Config defaults).
	 *
	 ...
	 * depend on Config can request a scoped cache without managing the
	 * factory directly.
	 *
	...
	public function get_runtime_cache($name, $driver = 'array') {
		return CacheFactory::named($name, $driver);
	}
	 ...
	 *
	 * Note: CacheFactory::make_driver() reads these settings via direct
	 * get_option() calls to avoid a bootstrapping circular dependency (Config
	 * internally relies on CacheFactory). All other callers that need these
