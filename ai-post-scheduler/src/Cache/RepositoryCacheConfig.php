...
class RepositoryCacheConfig {
	 ...
	 * @return Cache|null
	 */
	public static function resolve_cache_instance( string $group, array $policy ): ?Cache {
		$config = self::get_tier_config( self::policy_tier( $policy ) );
		...
		if (!empty( $config['persistent_allowed'] )) {
			return CacheFactory::named( $cache_name );
		}

		return CacheFactory::named( $cache_name, 'array' );
	}
