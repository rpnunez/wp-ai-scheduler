...
class RepositoryCacheObserver {
	 ...
	 * Constructor.
	 *
	 * @param LoggerInterface|null $logger Optional logger dependency.
	 */
	...
	/**
	 * Emit the normalized event through Telemetry when enabled.
	 *
	...
	private function record_telemetry(array $context) {
		try {
			...
			Telemetry::instance()->add_event('cache', $context);
		} catch (Throwable $e) {
	...
	private function get_logger() {
		...
		try {
			if (class_exists(Container::class)) {
				$logger = Container::get_instance()->make(LoggerInterface::class);
