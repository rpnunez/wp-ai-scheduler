...
use AIPS\Cache\RepositoryCacheConfig;
use AIPS\Cache\RepositoryCacheObserver;
use AIPS\Cache\RepositoryCacheKeyBuilder;
use AIPS\Cache\RepositoryCacheDependencies;
use Throwable;
...
trait CacheableRepositoryTrait {
	...
	protected function cache_read( string $operation_id, array $args, callable $callback, array $options = array() ) {
		...
		$cache = RepositoryCacheConfig::resolve_cache_instance( $this->repository_cache_group(), $policy );
		if (!$cache) {
		...
		$key          = RepositoryCacheKeyBuilder::build_key( $operation_id, $args, $tag_versions );
		$key_hash     = hash( 'sha256', $key );
		$observer     = $this->repository_cache_observer();
		$tier         = isset( $policy['tier'] ) ? (string) $policy['tier'] : RepositoryCacheConfig::TIER_NONE;
		...
		$cache->set(
			...
			RepositoryCacheConfig::resolve_ttl( $policy ),
	...
	protected function invalidate_cache_domain( string $domain, array $context = array(), string $reason = '' ) {
		...
		if (class_exists( RepositoryCacheDependencies::class ) && method_exists( RepositoryCacheDependencies::class, 'tags_for_invalidation' )) {
			$resolved_tags = RepositoryCacheDependencies::tags_for_invalidation( $domain, $context );
	 ...
	 *
	 * @return RepositoryCacheObserver
	 */
	protected function repository_cache_observer() {
		return new RepositoryCacheObserver();
	}
	...
	protected function repository_cache_should_bypass( array $policy, array $options = array() ): bool {
		...
		return RepositoryCacheConfig::TIER_NONE === ( isset( $policy['tier'] ) ? $policy['tier'] : RepositoryCacheConfig::TIER_NONE );
	}
	...
	private function repository_cache_tags( string $operation_id, array $policy, array $args ): array {
		...
		if (class_exists( RepositoryCacheDependencies::class ) && method_exists( RepositoryCacheDependencies::class, 'tags_for_read' )) {
			$dependency_tags = RepositoryCacheDependencies::tags_for_read( $operation_id, $args );
	 ...
	 *
	 * @param RepositoryCacheObserver $observer Observer instance.
	 * @param array                          $event Event payload.
	 * @return void
	 */
	private function record_repository_cache_read( RepositoryCacheObserver $observer, array $event ) {
	 ...
	 * @return void
	 */
	private function record_repository_cache_write( RepositoryCacheObserver $observer, array $event ) {
	 ...
	 * @return void
	 */
	private function record_repository_cache_bypass( RepositoryCacheObserver $observer, array $event ) {
	 ...
	 * @return void
	 */
	private function record_repository_cache_invalidation( RepositoryCacheObserver $observer, array $event ) {
	 ...
	 * @return void
	 */
	private function record_repository_cache_warning( RepositoryCacheObserver $observer, array $event ) {
	...
	private function resolve_bypass_reason( array $policy, array $options ): string {
		...
		if (RepositoryCacheConfig::TIER_NONE === ( isset( $policy['tier'] ) ? $policy['tier'] : RepositoryCacheConfig::TIER_NONE )) {
			return 'tier_none';
		}
	...
	private function repository_cache_invalidation_caches(): array {
		...
		foreach ( $this->repository_cache_policies() as $policy ) {
			...
			$policy = $this->normalize_repository_cache_policy( $policy );
			$tier   = isset( $policy['tier'] ) ? (string) $policy['tier'] : RepositoryCacheConfig::TIER_NONE;
			if (RepositoryCacheConfig::TIER_NONE === $tier) {
				continue;
			}
