 ...
 *
 * The table schema is managed by DBManager; this class never creates or
 * alters the table itself.
 *
 * @package AI_Post_Scheduler
 * @since   2.3.0
 */
class DBDriver implements CacheDriverInterface, CacheMonitorableDriverInterface {
	...
	public function get( $key, $group = 'default' ) {
		...
		if ((int) $row->expires_at > 0 && (int) $row->expires_at < AIPSDateTime::now()->timestamp()) {
	...
	public function set( $key, $value, $ttl = 0, $group = 'default' ) {
		...
		$now_ts = AIPSDateTime::now()->timestamp();
