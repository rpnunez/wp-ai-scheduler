...
use AIPS\Cache\CacheIndex;
use AIPS\Core\DateTime as AIPSDateTime;
use AIPS\Diagnostics\Telemetry;
 ...
 *
 * Wraps a concrete CacheDriverInterface implementation and exposes a
...
class Cache {
	 ...
	 *
	 * @var RepositoryCacheObserver|null
	 */
	 ...
	 * Constructor.
	 *
	 * @param CacheDriverInterface|null $driver Optional driver. When null, the
	 *                                        factory resolves the driver from
	 *                                        the current admin settings.
	 */
	public function __construct( CacheDriverInterface $driver = null, RepositoryCacheObserver $repository_cache_observer = null ) {
		if ($driver === null) {
			$driver = CacheFactory::make_driver();
		}
		$this->driver                    = $driver;
	 ...
	 */
	private function get_cache_index(): ?CacheIndex {
		...
		$this->cache_index = new CacheIndex();
		return $this->cache_index;
	}
	 ...
	 * going through Config) to avoid a bootstrapping circular dependency,
	...
	private static function is_system_enabled() {
		if (self::$system_enabled === null) {
			// Use get_option() directly — not Config — to avoid circular
			// dependency (Config itself uses Cache internally).
	...
	private function record_cache_event( $operation, array $data ) {
		if (!class_exists( Telemetry::class ) || !Telemetry::is_enabled()) {
			return;
		}

		$driver = get_class( $this->driver );
		Telemetry::instance()->add_event(
			'cache',
			array_merge(
				array(
	 ...
	 *
	 * @return RepositoryCacheObserver|null
	 */
	private function get_repository_cache_observer() {
		if ($this->repository_cache_observer instanceof RepositoryCacheObserver) {
			return $this->repository_cache_observer;
		}

		if (!class_exists( RepositoryCacheObserver::class )) {
			return null;
		}

		$this->repository_cache_observer = new RepositoryCacheObserver();
