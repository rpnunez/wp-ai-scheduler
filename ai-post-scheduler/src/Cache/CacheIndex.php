 ...
 * Cache::set(). This makes cache entries enumerable even when the
...
class CacheIndex {
	...
	public function __construct() {
		// Use get_option() directly to avoid routing through Config's internal
		// Cache instance. Config::get_option() with a non-null default
		...
		// which previously called back into Config::get_option() and recursed.
	...
	// -----------------------------------------------------------------------
	// Public API (called by Cache hooks)
	// -----------------------------------------------------------------------
	 ...
	 * @param array  $context  Optional context metadata passed by Cache::set().
	 * @return void
	 */
	...
	public function record_access( string $key, string $group ): void {
		...
		try {
			...
			$table    = $wpdb->prefix . 'aips_cache_index';
			$key_hash = hash( 'sha256', $group . ':' . $key );
			$now      = AIPSDateTime::now()->timestamp();
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery
	...
	private function upsert_index_row( string $key, $value, int $ttl, string $group, array $context ): void {
		...
		// Use get_option() directly to avoid routing through Config's internal
		// Cache instance, which would trigger record_set() recursively.
	 ...
	 * cache writes are frequent (or recursive through Config), exhaust
