...
use AIPS\Templates\TemplateRepository;
use AIPS\Scheduler\ScheduleService;
use AIPS\AI\AIServiceInterface;
use AIPS\AI\AIService;
use AIPS\Routing\AJAXResponse;
use AIPS\Services\VoicesRepository;
use AIPS\Services\ArticleStructureRepository;
use AIPS\Author\AuthorsRepository;
use AIPS\Scheduler\IntervalCalculator;
use AIPS\History\HistoryService;
use AIPS\Admin\AdminMenuHelper;
...
class CampaignsController {
	...
	public function __construct(
		?CampaignsRepository $campaigns_repository = null,
		?TemplateRepository $template_repository = null,
		?ScheduleService $unified_schedule_service = null,
		?Config $config = null,
		?AIServiceInterface $ai_service = null
	) {
		$container = Container::get_instance();

		$this->campaigns_repository = $campaigns_repository ?: $container->makeIfExists(CampaignsRepository::class, function() {
			return CampaignsRepository::instance();
		});
		$this->template_repository = $template_repository ?: $container->makeIfExists(TemplateRepository::class, function() {
			return TemplateRepository::instance();
		});
		$this->unified_schedule_service = $unified_schedule_service ?: $container->makeIfExists(ScheduleService::class, function() {
			return new ScheduleService();
		});
		$this->config = $config ?: $container->makeIfExists(Config::class, function() {
	...
	public function render_wizard_page() {
		...
		$voices          = (new VoicesRepository())->get_all();
		$structures      = (new ArticleStructureRepository())->get_all(true);
		$authors         = AuthorsRepository::instance()->get_all(true);
		$frequencies     = (new IntervalCalculator())->get_intervals();
	...
	private function record_campaign_lifecycle_activity($campaign_id, $event_type, $action_label) {
		$history_service = new HistoryService();
	...
	public function ajax_get_campaign_metrics() {
		...
		if (!$campaign_id) {
			AJAXResponse::error(__('Invalid campaign ID.', 'ai-post-scheduler'), 'invalid_id', 400);
		}
	...
	public function ajax_toggle_campaign() {
		...
		if (is_wp_error($result)) {
			AJAXResponse::error($result->get_error_message(), $result->get_error_code(), $this->map_campaign_error_status($result));
		}

		if ($result) {
			$message = $is_active ? __('Campaign resumed.', 'ai-post-scheduler') : __('Campaign paused.', 'ai-post-scheduler');
			AJAXResponse::success(array('is_active' => $is_active), $message);
		}

		AJAXResponse::error(__('Failed to update campaign status.', 'ai-post-scheduler'), 'update_failed', 500);
	}
	...
	public function ajax_duplicate_campaign() {
		...
		if (is_wp_error($new_campaign_id)) {
			AJAXResponse::error($new_campaign_id->get_error_message(), $new_campaign_id->get_error_code(), $this->map_campaign_error_status($new_campaign_id));
		}
	...
	public function ajax_archive_campaign() {
		...
		if ($result) {
			AJAXResponse::success(array(), __('Campaign archived successfully.', 'ai-post-scheduler'));
		}

		AJAXResponse::error(__('Failed to archive campaign.', 'ai-post-scheduler'), 'archive_failed', 500);
	}
	...
	public function ajax_restore_campaign() {
		...
		if ($result) {
			AJAXResponse::success(array(), __('Campaign restored successfully.', 'ai-post-scheduler'));
		}

		AJAXResponse::error(__('Failed to restore campaign.', 'ai-post-scheduler'), 'restore_failed', 500);
	}
	...
	public function ajax_delete_campaign() {
		...
		if (!$this->campaigns_repository->can_delete_campaign($campaign_id)) {
			AJAXResponse::error(__('This campaign has generated posts and can only be archived.', 'ai-post-scheduler'), 'delete_blocked', 400);
		}
		...
		if ($result) {
			AJAXResponse::success(array(), __('Campaign deleted successfully.', 'ai-post-scheduler'));
		}

		AJAXResponse::error(__('Failed to delete campaign.', 'ai-post-scheduler'), 'delete_failed', 500);
	}
	...
	public function ajax_finalize_campaign() {
		...
		AJAXResponse::success(array(
			...
			'summary' => $this->build_summary($payload),
			'redirect_url' => AdminMenuHelper::get_page_url('campaigns'),
	...
	public function ajax_ai_generate_campaign() {
		...
		if (!$this->ai_service || !$this->ai_service->is_available()) {
			AJAXResponse::error(__('AI Engine is not available.', 'ai-post-scheduler'), 'ai_unavailable', 503);
		}
		...
		if (is_wp_error($response)) {
			AJAXResponse::error($response->get_error_message(), 'ai_generation_failed', 500);
		}

		if (!is_array($response)) {
			AJAXResponse::error(__('AI response was not valid JSON.', 'ai-post-scheduler'), 'invalid_ai_response', 500);
		}
	...
	private function get_ai_intake_payload() {
		...
		$interval_calculator = new IntervalCalculator();
	...
	private function build_ai_campaign_prompt($intake) {
		$context = array(
			'intake' => $intake,
			'available_frequencies' => array_keys((new IntervalCalculator())->get_intervals()),
	...
	private function prepare_ai_payload($response, $intake) {
		...
		return array(
			...
			'is_active' => 1,
			'start_time' => DateTime::now()->toDisplay('Y-m-d\TH:i'),
		);
	}
	...
	private function build_ai_strategy_preview($draft, $intake, $response) {
		...
		$intervals = (new IntervalCalculator())->get_intervals();
	 ...
	 * 2) Container binding for AIServiceInterface
	 * 3) New AIService instance
	 *
	 * @param AIServiceInterface|null $ai_service Optional injected AI service.
	 * @return AIServiceInterface
	 */
	private function resolve_ai_service($ai_service) {
		if ($ai_service instanceof AIServiceInterface) {
			return $ai_service;
		}

		$container = Container::get_instance();
		$resolved = $container->makeIfExists(AIServiceInterface::class, function() {
			return new AIService();
		});
		if ($resolved instanceof AIServiceInterface) {
			return $resolved;
		}
	...
	private function normalise_payload($payload) {
		...
		return array(
			...
			'start_time'             => isset($payload['start_time']) ? sanitize_text_field($payload['start_time']) : DateTime::now()->toDisplay('Y-m-d\TH:i'),
	...
	private function find_unified_schedule($schedule_id) {
		$schedules = $this->unified_schedule_service->get_all(ScheduleService::TYPE_TEMPLATE, false);
