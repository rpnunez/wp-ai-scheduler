...
use AIPS\Scheduler\ScheduleRepository;
use AIPS\Scheduler\Scheduler;
...
class CampaignsRepository {
	...
	public function __construct($template_repository = null, $schedule_repository = null) {
		...
		$this->template_repository = $template_repository ?: TemplateRepository::instance();
		$this->schedule_repository = $schedule_repository ?: ScheduleRepository::instance();
		$this->cache = null;
	...
	public function duplicate_campaign($campaign_id) {
		...
		foreach ($schedules as $schedule) {
			...
			$schedule_data['next_run'] = DateTime::now()->timestamp();

			$new_schedule_id = (new Scheduler())->save_schedule($schedule_data);
	...
	private function get_cache() {
		...
		try {
			$this->cache = CacheFactory::make('db');
		} catch (Throwable $e) {
			$this->cache = null;
			if (class_exists(Logger::class)) {
				Logger::instance()->warning('Failed to initialize DB cache for campaign metrics; continuing without cache.', array(
					'error' => $e->getMessage(),
