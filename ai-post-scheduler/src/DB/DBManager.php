 ...
 * Database Manager
 *
 * Handles plugin database schema installation, maintenance, repairs, and migrations.
 *
...
class DBManager {
	 ...
	 *
	 * This is the single canonical implementation. Both DBMigrations and
	 * DateTimeDBRepair delegate here rather than duplicating the logic.
	 *
	...
	public function ajax_repair_db() {
		...
		if (!current_user_can('manage_options')) {
			AJAXResponse::error('Unauthorized');
		}

		self::install_tables();
		AJAXResponse::success(array('message' => 'Database tables repaired successfully.'));
	}
	...
	public function ajax_reinstall_db() {
		...
		AJAXResponse::success(array('message' => 'Database tables reinstalled successfully.'));
	}
	...
	public function ajax_wipe_db() {
		...
		AJAXResponse::success(array('message' => 'Plugin data wiped successfully.'));
	}
	...
	public function ajax_flush_cron_events() {
		...
		if (!current_user_can('manage_options')) {
			AJAXResponse::error(__('Unauthorized', 'ai-post-scheduler'));
		}
