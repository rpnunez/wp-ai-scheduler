 ...
 * ─────────────────────
 * Layer 1 — DBManager  (schema truth via dbDelta)
 ...
 *
 * Layer 2 — DBMigrations  (this class)
 ...
 *   in DBManager::get_schema(); no CREATE TABLE SQL belongs here.
 *
...
use AIPS\Core\DateTime as AIPSDateTime;
...
class DBMigrations {
	...
	public static function check_and_run() {
		$current_version = Config::get_instance()->get_option( 'aips_db_version' );
	 ...
	 *   2. DBManager::install_tables() runs second — dbDelta applies
	 ...
	 *   4. aips_db_version is stamped to AIPS_VERSION via Config::set_option()
	...
	private function run_upgrade( $from_version ) {
		...
		$install_result = DBManager::install_tables();
		...
		// Use Config::set_option() so the per-request option cache is
		// invalidated immediately; bare update_option() would leave the cache
		// stale for the rest of this request.
		Config::get_instance()->set_option( 'aips_db_version', AIPS_VERSION );
	 ...
	 * single UTC-based integer representation so DateTime is the single
	 * entry point for all date/time operations.
	 *
	 * All live sites running versions prior to 2.5.0 have had this migration
	 * applied. The SHOW COLUMNS guards in DBManager::convert_datetime_column_to_bigint()
	...
	private function migrate_to_2_5_0() {
		...
		foreach ( DBManager::get_datetime_column_map() as $table_slug => $columns ) {
			...
			foreach ( $columns as $col_def ) {
				DBManager::convert_datetime_column_to_bigint( $table, $col_def[0] );
			}
		}
	}
	...
	private function migrate_to_2_8_2() {
		...
		foreach ( $schedules as $schedule ) {
			$created_at = ! empty( $schedule['created_at'] ) ? absint( $schedule['created_at'] ) : AIPSDateTime::now()->timestamp();
			$updated_at = AIPSDateTime::now()->timestamp();
	...
	private function migrate_to_2_8_3() {
		$summary = ( new DateTimeDBRepair() )->run();
	...
	private function repair_schedule_last_run_from_run_state() {
		...
		foreach ( $rows as $row ) {
			...
			if ( $current_last_run >= DateTimeDBRepair::MIN_VALID_TIMESTAMP ) {
				continue;
			}
			...
			if ( $run_at_ts < DateTimeDBRepair::MIN_VALID_TIMESTAMP ) {
				continue;
			}
	...
	private function repair_template_schedule_next_runs_from_last_run() {
		...
		foreach ( $rows as $row ) {
			...
			if ( $last_run < DateTimeDBRepair::MIN_VALID_TIMESTAMP ) {
				continue;
			}
	...
	private function repair_author_schedule_next_runs_from_last_run() {
		...
		foreach ( $rows as $row ) {
			...
			if ( $topic_last_run >= DateTimeDBRepair::MIN_VALID_TIMESTAMP ) {
			...
			if ( $post_last_run >= DateTimeDBRepair::MIN_VALID_TIMESTAMP ) {
