...
use AIPS\Repositories\ArticleStructureRepository;
use AIPS\Repositories\PromptSectionRepository;
...
/**
 * Developer tools for creating template scaffolds.
 *
...
class DevTools {
    ...
    public function ajax_generate_scaffold() {
        ...
        if ($include_voice && isset($data['voice'])) {
            $voices_handler = new \AIPS\Admin\Voices();
            $voice_data = array(
        ...
        if ($include_structure && isset($data['article_structure'])) {
            $structure_repo = new ArticleStructureRepository();
            $section_repo = new PromptSectionRepository();
