...
class TopicPenaltyService {
    ...
    public function apply_penalty($topic_id, $reason_category) {
        ...
        if ($result !== false) {
            $this->logger->log(
                sprintf(
                    'Applied %s penalty (%d points) to topic %d. New score: %d',
                    $reason_category,
                    $penalty,
                    $topic_id,
                    $new_score
    ...
    public function apply_reward($topic_id, $reason_category) {
        ...
        if ($result !== false) {
            $this->logger->log(
                sprintf(
                    'Applied %s reward (+%d points) to topic %d. New score: %d',
                    $reason_category,
                    $reward,
                    $topic_id,
                    $new_score
    ...
    private function flag_author_for_policy_review($author_id, $topic_id) {
        ...
        if (count($metadata['policy_flags']) >= 3) {
            $this->logger->log(
                sprintf(
                    'Author %d has %d policy violations. Consider deactivation.',
                    $author_id,
