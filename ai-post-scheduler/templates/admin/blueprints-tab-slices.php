<?php
/**
 * Blueprints tab: Slices
 *
 * Renders the Post Slices list within the Blueprints tabbed page.
 *
...
if (!isset($post_slice_counts) || !is_array($post_slice_counts)) {
	$post_slice_counts = array('total' => count($post_slices), 'active' => 0, 'inactive' => 0);
}
...
<div class="aips-content-panel">
	...
	<div class="aips-panel-body no-padding">
		<table class="aips-table aips-post-slices-table" id="aips-post-slices-table">
			...
			<tbody>
				...
				?>
				<tr data-slice-id="<?php echo esc_attr($slice_id); ?>" data-name="<?php echo esc_attr($name); ?>" data-description="<?php echo esc_attr($description); ?>" data-sort-order="<?php echo esc_attr($sort_order); ?>" data-active="<?php echo esc_attr($is_active); ?>">
					<td class="column-name cell-primary"><?php echo esc_html($name); ?></td>
					<td class="column-description">
						<?php echo '' !== $description ? esc_html($description) : '<span class="cell-meta">' . esc_html__('No description', 'ai-post-scheduler') . '</span>'; ?>
					</td>
					<td class="column-sort-order"><?php echo esc_html($sort_order); ?></td>
					...
					<td class="column-actions">
						<div class="aips-action-buttons">
							<button type="button" class="aips-btn aips-btn-sm aips-edit-post-slice" data-id="<?php echo esc_attr($slice_id); ?>" title="<?php esc_attr_e('Edit', 'ai-post-scheduler'); ?>">
								<span class="dashicons dashicons-edit"></span>
							</button>
							<button type="button" class="aips-btn aips-btn-sm aips-btn-ghost aips-toggle-post-slice" data-id="<?php echo esc_attr($slice_id); ?>" data-active="<?php echo esc_attr($is_active); ?>">
								<span class="dashicons <?php echo $is_active ? 'dashicons-hidden' : 'dashicons-visibility'; ?>"></span>
							</button>
							<button type="button" class="aips-btn aips-btn-sm aips-btn-danger aips-delete-post-slice" data-id="<?php echo esc_attr($slice_id); ?>" title="<?php esc_attr_e('Delete', 'ai-post-scheduler'); ?>">
	...
	<div class="tablenav">
		<span class="aips-table-footer-count">
			<?php printf(esc_html(_n('%s post slice', '%s post slices', count($post_slices), 'ai-post-scheduler')), esc_html((string) count($post_slices))); ?>
		</span>
	</div>
	<?php else: ?>
	<div class="aips-empty-state" id="aips-post-slices-empty">
		...
		<p class="aips-empty-state-description"><?php esc_html_e('Create post slices to steer generated posts toward specific editorial styles.', 'ai-post-scheduler'); ?></p>
		<div class="aips-empty-state-actions">
			<button type="button" class="aips-btn aips-btn-primary" id="aips-add-post-slice-empty-btn"><?php esc_html_e('Add Your First Post Slice', 'ai-post-scheduler'); ?></button>
		</div>
	</div>
	<?php endif; ?>
</div>
...
<div id="aips-post-slice-modal" class="aips-modal" style="display:none;" role="dialog" aria-modal="true" aria-labelledby="aips-post-slice-modal-title">
	<div class="aips-modal-content">
		...
		<div class="aips-modal-body">
			<form id="aips-post-slice-form" novalidate>
				...
				<div class="aips-form-row">
					<label for="aips-post-slice-name"><?php esc_html_e('Slice Name', 'ai-post-scheduler'); ?> <span class="required">*</span></label>
				...
				<div class="aips-form-row">
					...
					<textarea id="aips-post-slice-description" name="description" rows="4" class="large-text"></textarea>
				</div>
				<div class="aips-form-row">
		...
		<div class="aips-modal-footer">
			...
			<button type="button" class="button button-primary" id="aips-save-post-slice-btn"><?php esc_html_e('Save Post Slice', 'ai-post-scheduler'); ?></button>
		</div>
	</div>
</div>
