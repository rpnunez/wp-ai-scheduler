...
$author_config = AIPS_Config::get_instance();
$author_allowed_post_types = $author_config->get_generation_post_types();
$author_default_post_type = $author_config->get_default_generation_post_type();
$author_selectable_post_types = AIPS_Utilities::get_selectable_post_types();
?>
                            ...
                            ?>
                                <tr data-author-id="<?php echo esc_attr($author->id); ?>" data-field-niche="<?php echo esc_attr($author->field_niche); ?>">
                                    ...
                                    <td>
                                        ...
                                        <?php endif; ?>
                                        <?php if (!empty($author->topic_auto_approval_mode) && 'manual' !== $author->topic_auto_approval_mode): ?>
                                        <div style="margin-top: 4px;">
                                            <span class="aips-badge aips-badge-info" title="<?php
                                                if ($author->topic_auto_approval_mode === 'all') {
                                                    esc_attr_e('Auto-approves all generated topics', 'ai-post-scheduler');
                                                } elseif ($author->topic_auto_approval_mode === 'score') {
                                                    echo esc_attr(sprintf(__('Auto-approves topics with score ≥ %d', 'ai-post-scheduler'), isset($author->topic_auto_approval_min_score) ? (int) $author->topic_auto_approval_min_score : 70));
                                                } elseif ($author->topic_auto_approval_mode === 'similarity') {
                                                    echo esc_attr(sprintf(__('Auto-approves topics with duplicate similarity < %.0f%%', 'ai-post-scheduler'), (isset($author->topic_auto_approval_max_similarity) ? (float) $author->topic_auto_approval_max_similarity : 0.8) * 100));
                                                }
                                            ?>">
                                                <span class="dashicons dashicons-controls-play"></span>
                                                <?php
                                                if ($author->topic_auto_approval_mode === 'all') {
                                                    esc_html_e('Auto-Approve All', 'ai-post-scheduler');
                                                } elseif ($author->topic_auto_approval_mode === 'score') {
                                                    printf(esc_html__('Auto: Score ≥ %d', 'ai-post-scheduler'), (int) $author->topic_auto_approval_min_score);
                                                } elseif ($author->topic_auto_approval_mode === 'similarity') {
                                                    printf(esc_html__('Auto: Sim < %.0f%%', 'ai-post-scheduler'), ((float) $author->topic_auto_approval_max_similarity) * 100);
                                                }
                                                ?>
                                            </span>
                                        </div>
...
<div id="aips-author-modal" class="aips-modal" style="display: none;">
    <div class="aips-modal-content">
        ...
        <form id="aips-author-form">
            ...
            <div class="form-group">
                <label for="author_post_type"><?php esc_html_e('Post Type', 'ai-post-scheduler'); ?></label>
                <select id="author_post_type" name="post_type">
                    <option value="default"><?php printf(esc_html__('— Use Global Default (%s) —', 'ai-post-scheduler'), esc_html(isset($author_selectable_post_types[$author_default_post_type]['label']) ? $author_selectable_post_types[$author_default_post_type]['label'] : $author_default_post_type)); ?></option>
                    <?php foreach ($author_selectable_post_types as $pt_slug => $pt_info): ?>
                        <?php if (in_array($pt_slug, $author_allowed_post_types, true)): ?>
                            <option value="<?php echo esc_attr($pt_slug); ?>">
                                <?php echo esc_html($pt_info['label']); ?> (<?php echo esc_html($pt_slug); ?>)
                            </option>
                        <?php endif; ?>
                    <?php endforeach; ?>
                </select>
                <p class="description"><?php esc_html_e('Which WordPress post type this author generates. Defaults to the global default post type.', 'ai-post-scheduler'); ?></p>
            </div>
