...
class AIPS_History_Repository implements AIPS_History_Repository_Interface {
	...
	public function get_stats_in_period( $from_ts, $to_ts ) {
		...
		$query = $this->wpdb->prepare(
			...
			array_merge($auxiliary_methods, array((int) $from_ts, (int) $to_ts))
		);
		return $this->wpdb->get_row($query);
	}

	/**
	 * Get schedules run count in a period.
	 *
	 * @param int $from_ts Start timestamp.
	 * @param int $to_ts End timestamp.
	 * @return int
	 */
	public function get_schedules_run_count_in_period( $from_ts, $to_ts ) {
	...
	/**
	 * Get AI calls and errors in a period.
	 *
	 * @param int $from_ts Start timestamp.
	 * @param int $to_ts End timestamp.
	 * @return object
	 */
	public function get_ai_calls_and_errors_in_period( $from_ts, $to_ts ) {
		return $this->wpdb->get_row($this->wpdb->prepare(
			"SELECT
	 ...
	 * @return array
	 */
	public function get_recent_posts_in_period( $from_ts, $to_ts, $limit = 10 ) {
		$auxiliary_methods = $this->get_auxiliary_creation_methods();
		$auxiliary_placeholders = implode(', ', array_fill(0, count($auxiliary_methods), '%s'));
		$templates_table = $this->wpdb->prefix . 'aips_templates';
		...
		$query = $this->wpdb->prepare(
			"SELECT h.id, h.uuid, h.correlation_id, h.post_id, h.template_id, h.campaign_id, h.topic_id, h.status, h.generated_title, h.created_at, h.completed_at, h.creation_method,
			 ...
			 LIMIT %d",
			array_merge(array((int) $from_ts, (int) $to_ts), $auxiliary_methods, array((int) $limit))
		);
		return $this->wpdb->get_results($query);
	}

	/**
	 * Get posts generated by topic in a period.
	 *
	 * @param int $from_ts Start timestamp.
	 * @param int $to_ts End timestamp.
	 * @param int $limit Max rows to return.
	 * @return array
	 */
	public function get_posts_by_topic_in_period( $from_ts, $to_ts, $limit = 10 ) {
		$table_author_topics = $this->wpdb->prefix . 'aips_author_topics';
	...
	/**
	 * Get executed schedules in a period.
	 *
	 * @param int $from_ts Start timestamp.
	 * @param int $to_ts End timestamp.
	 * @param int $limit Max rows to return.
	 * @return array
	 */
	public function get_executed_schedules_in_period( $from_ts, $to_ts, $limit = 10 ) {
		$table_schedule = $this->wpdb->prefix . 'aips_schedule';
		$table_templates = $this->wpdb->prefix . 'aips_templates';
		$table_authors = $this->wpdb->prefix . 'aips_authors';

		return $this->wpdb->get_results($this->wpdb->prepare(
			"SELECT h.id, h.uuid, h.status, h.completed_at, h.created_at, h.creation_method,
					MAX(s.title) as schedule_title, t.name as template_name, a.name as author_name
			 FROM {$this->table_name} h
			 LEFT JOIN {$table_schedule} s ON (
				 (h.creation_method = 'scheduled' AND h.template_id = s.template_id)
				 OR (h.creation_method IN ('author_topic_gen', 'author_post_gen') AND h.author_id = s.author_id)
			 )
	...
	/**
	 * Get daily generation stats in a period.
	 *
	 * @param int $from_ts Start timestamp.
	 * @param int $to_ts End timestamp.
	 * @return array
	 */
	public function get_daily_generation_stats_in_period( $from_ts, $to_ts ) {
		$auxiliary_methods = $this->get_auxiliary_creation_methods();
		$auxiliary_placeholders = implode(', ', array_fill(0, count($auxiliary_methods), '%s'));
		$query_args = array_merge(array((int) $from_ts, (int) $to_ts), $auxiliary_methods);
	...
	/**
	 * Get daily AI stats in a period.
	 *
	 * @param int $from_ts Start timestamp.
	 * @param int $to_ts End timestamp.
	 * @return array
	 */
	public function get_daily_ai_stats_in_period( $from_ts, $to_ts ) {
		return $this->wpdb->get_results($this->wpdb->prepare(
