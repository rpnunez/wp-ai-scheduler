...
class AIPS_Scheduler {
    ...
    /**
     * @var AIPS_Generator Generator instance (dependency injection support)
     */
    private $generator;
    ...
    /**
     * Set the generator instance (for testing).
     *
    ...
    public function process_scheduled_posts() {
        ...
        $generator = $this->generator ? $this->generator : new AIPS_Generator();
        ...
        foreach ($due_schedules as $schedule) {
            try {
                ...
                // Catch any PHP errors or exceptions to prevent blocking other schedules
                $logger->log('CRITICAL: Unexpected error processing schedule ' . $schedule->schedule_id . ': ' . $e->getMessage(), 'error', array(
                ...
                $this->activity_repository->create(array(
                    ...
                    'template_id' => $schedule->template_id,
                    'message' => sprintf(
                        __('Schedule "%s" crashed with critical error', 'ai-post-scheduler'),
                        $schedule->template_name
                    ),
                    'metadata' => array(
                        'error' => $e->getMessage(),
                ...
                do_action('aips_schedule_execution_failed', $schedule->schedule_id, 'Critical error: ' . $e->getMessage());
            }
        }
    }
}
