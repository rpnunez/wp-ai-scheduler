    ...
    public function process_scheduled_posts() {
        ...
        foreach ($due_schedules as $schedule) {
            try {
                ...
                $logger->log('Critical error processing schedule ' . $schedule->schedule_id . ': ' . $e->getMessage(), 'error');
                
                // Log critical failure to activity feed
                $this->activity_repository->create(array(
                    'event_type' => 'schedule_failed',
                    'event_status' => 'failed',
                    'schedule_id' => $schedule->schedule_id,
                    'template_id' => $schedule->template_id,
                    'message' => sprintf(
                        __('Schedule "%s" encountered a critical error', 'ai-post-scheduler'),
                        $schedule->name
                    ),
                    'metadata' => array(
                        'error' => $e->getMessage(),
