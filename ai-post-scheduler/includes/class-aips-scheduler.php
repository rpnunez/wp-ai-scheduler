    ...
    public function process_scheduled_posts() {
        ...
        foreach ($due_schedules as $schedule) {
            try {
                ...
                if (!$lock_result) {
                    $logger->log('Failed to acquire lock for schedule ' . $schedule->schedule_id . ' (already claimed)', 'info');
                    continue; // Skip generation if we couldn't lock (likely claimed by another process)
                }
