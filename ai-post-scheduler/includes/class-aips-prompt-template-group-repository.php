 ...
 * Manages only the aips_prompt_template_groups table; per-component item
 * persistence belongs in AIPS_Prompt_Template_Item_Repository.
 *
 * This class additionally owns the runtime prompt-resolution entry point
 * (get_prompt_for_component) and its in-request static cache, because that
 * lookup must coordinate group selection with item retrieval.
 *
 ...
 *
 * Provides CRUD operations for prompt template groups and the
 * get_prompt_for_component() helper used by all Prompt Builder classes.
 */
class AIPS_Prompt_Template_Group_Repository {
	...
	/**
	 * @var string Groups table name (with WordPress table prefix).
	 */
	...
	/**
	 * @var AIPS_Prompt_Template_Item_Repository Item repository for cascaded operations.
	 */
	private $item_repo;
	...
	private function load_default_group_prompts() {
		...
		$items = $this->item_repo->get_items_for_group( (int) $group->id );
	 ...
	 *
	 * Automatically populates all component items via the item repository so
	...
	public function create_group( array $data ) {
		...
		// Seed all components with built-in defaults via the item repository.
		$this->item_repo->seed_default_items( $group_id );
	...
	/**
	 * Update an existing group's name/description/default status.
	 *
	 * @param int   $id   Group ID.
	...
	public function delete_group( $id ) {
		...
		// Delete component items first via the item repository.
		$this->item_repo->delete_items_for_group( $id );
	 ...
	 *
	 * Creates the "Default" group (from AIPS_Prompt_Template_Defaults) with all
	 * built-in component prompts seeded if no groups are present.  Safe to call
	 * multiple times; is a no-op when groups already exist.
	 *
	 * @return void
	 */
	public function ensure_default_group_exists() {
		...
		$defaults = AIPS_Prompt_Template_Defaults::get_default_group();
