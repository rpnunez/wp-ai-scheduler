...
class AIPS_Embeddings_Cron {
	...
	private function schedule_next_batch($author_id, $batch_size, $last_processed_id) {
		...
		// Route through the centralized job scheduler; the resolved transport
		// (Action Scheduler when available, otherwise WP-Cron) is chosen in
		// infrastructure wiring, not here. The callback receives a single
		// associative $args array under both transports.
