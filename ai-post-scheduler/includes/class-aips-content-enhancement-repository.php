...
class AIPS_Content_Enhancement_Repository {
	...
	private function normalize( array $data, string $id, int $now ): array {
		...
		return array(
			...
			'cta_text'        => sanitize_text_field( $data['cta_text'] ?? $data['cta_label'] ?? $config->get_option( 'aips_content_enhancement_default_cta_text' ) ),
			'cta_label'       => sanitize_text_field( $data['cta_label'] ?? $data['cta_text'] ?? $config->get_option( 'aips_content_enhancement_default_cta_text' ) ),
