 ...
 * delegates persistence to AIPS_Prompt_Template_Group_Repository.
 */
class AIPS_Prompt_Templates_Controller {
	...
	public function __construct() {
		$this->repo = AIPS_Prompt_Template_Group_Repository::instance();
	...
	public function ajax_get_groups() {
		...
		$this->repo->ensure_default_group_exists();

		$groups = $this->repo->get_all_groups();
	...
	public function ajax_get_group() {
		...
		$group = $this->repo->get_group( $id );
		if ( ! $group ) {
			wp_send_json_error( array( 'message' => __( 'Group not found.', 'ai-post-scheduler' ) ), 404 );
		}

		$items      = $this->repo->get_items_for_group( $id );
		$components = $this->repo->get_component_definitions();
	...
	public function ajax_save_group() {
		...
		if ( $id ) {
			// Update.
			$ok = $this->repo->update_group( $id, array(
				'name'        => $name,
			...
			// Create.
			$new_id = $this->repo->create_group( array(
				'name'        => $name,
				'description' => $description,
				'is_default'  => $is_default,
			) );
			...
			$group = $this->repo->get_group( $new_id );
			wp_send_json_success( array(
				'group'   => $group,
	...
	public function ajax_delete_group() {
		...
		// If deleting the default group, promote another one.
		if ( (int) $group->is_default === 1 ) {
			$others = $this->repo->get_all_groups();
			foreach ( $others as $other ) {
				if ( (int) $other->id !== $id ) {
					$this->repo->set_default_group( (int) $other->id );
					break;
				}
			}
		}

		$ok = $this->repo->delete_group( $id );
		if ( ! $ok ) {
	...
	public function ajax_set_default_group() {
		...
		$ok = $this->repo->set_default_group( $id );
		if ( ! $ok ) {
	...
	public function ajax_save_items() {
		...
		if ( ! $this->repo->get_group( $group_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Group not found.', 'ai-post-scheduler' ) ), 404 );
		}
		...
		$definitions = $this->repo->get_component_definitions();
		$clean_items = array();
		...
		$ok = $this->repo->save_items( $group_id, $clean_items );
		if ( ! $ok ) {
