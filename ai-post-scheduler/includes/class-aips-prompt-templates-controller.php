...
class AIPS_Prompt_Templates_Controller {
	...
	public function ajax_get_groups() {
		...
		AIPS_Ajax_Response::success( array( 'groups' => $groups ) );
	}
	...
	public function ajax_get_group() {
		...
		if ( ! $id ) {
			AIPS_Ajax_Response::invalid_request( __( 'Invalid group ID.', 'ai-post-scheduler' ) );
		}

		$group = $this->group_repo->get_group( $id );
		if ( ! $group ) {
			AIPS_Ajax_Response::not_found( __( 'Group', 'ai-post-scheduler' ) );
		}
	...
	public function ajax_save_group() {
		...
		if ( $name === '' ) {
			AIPS_Ajax_Response::invalid_request( __( 'Group name is required.', 'ai-post-scheduler' ) );
		}
		...
		if ( $id ) {
			...
			if ( ! $ok ) {
				AIPS_Ajax_Response::error(
					__( 'Failed to update group.', 'ai-post-scheduler' ),
					'update_failed',
					500
				);
			}
			...
			AIPS_Ajax_Response::success(
				array( 'group' => $group ),
				__( 'Group updated successfully.', 'ai-post-scheduler' )
			);
		} else {
			// Create.
			...
			if ( ! $new_id ) {
				AIPS_Ajax_Response::error(
					__( 'Failed to create group.', 'ai-post-scheduler' ),
					'create_failed',
					500
				);
			}
			...
			AIPS_Ajax_Response::success(
				array( 'group' => $group ),
				__( 'Group created successfully.', 'ai-post-scheduler' )
			);
		}
	}
	...
	public function ajax_delete_group() {
		...
		if ( $this->group_repo->count_groups() <= 1 ) {
			AIPS_Ajax_Response::error(
				__( 'Cannot delete the last remaining group.', 'ai-post-scheduler' ),
				'last_group',
				400
			);
		}
		...
		if ( ! $ok ) {
			AIPS_Ajax_Response::error(
				__( 'Failed to delete group.', 'ai-post-scheduler' ),
				'delete_failed',
				500
			);
		}

		AIPS_Ajax_Response::success( array(), __( 'Group deleted.', 'ai-post-scheduler' ) );
	}
	...
	public function ajax_set_default_group() {
		...
		if ( ! $ok ) {
			AIPS_Ajax_Response::error(
				__( 'Failed to set default group.', 'ai-post-scheduler' ),
				'set_default_failed',
				500
			);
		}

		AIPS_Ajax_Response::success( array(), __( 'Default group updated.', 'ai-post-scheduler' ) );
	}
	...
	public function ajax_save_items() {
		...
		if ( ! is_array( $items ) ) {
			AIPS_Ajax_Response::invalid_request( __( 'Invalid items data.', 'ai-post-scheduler' ) );
		}
		...
		if ( ! $ok ) {
			AIPS_Ajax_Response::error(
				__( 'Failed to save some items.', 'ai-post-scheduler' ),
				'save_failed',
				500
			);
		}
		...
		AIPS_Ajax_Response::success( array(), __( 'Prompt templates saved.', 'ai-post-scheduler' ) );
	}
