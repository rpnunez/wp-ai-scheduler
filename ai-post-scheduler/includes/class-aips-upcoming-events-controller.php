...
class AIPS_Upcoming_Events_Controller {
	...
	public function render_page() {
		...
		$generated_at     = AIPS_DateTime::now()->toDisplay(get_option('date_format') . ' ' . get_option('time_format'));
		$auto_refresh     = !empty($filter_state['auto_refresh']);
	...
	private function get_filter_state() {
		...
		return array(
			...
			'search'   => $search,
			'auto_refresh' => $auto_refresh > 0 ? 1 : 0,
		);
	}
