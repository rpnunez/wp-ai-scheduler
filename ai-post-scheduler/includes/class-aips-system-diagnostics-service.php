...
class AIPS_System_Diagnostics_Service {
     ...
     * @return int
     */
    public function count_cron_hook_instances( $hook ) {
     ...
     */
    public function get_cron_hook_timestamps( $hook ) {
        $cron_array = _get_cron_array();
        if ( ! is_array( $cron_array ) ) {
            return array();
        }
     ...
     */
    public function check_scheduler_health() {
        $checks = array();
     ...
     * @return array<string, array<string, mixed>>
     */
    public function check_generation_metrics() {
        if ( ! class_exists( 'AIPS_Metrics_Repository' ) ) {
            return array(
                'unavailable' => array(
     ...
     * @return array<string, array<string, mixed>>
     */
    public function check_resilience() {
    ...
    public function check_environment() {
        ...
        return array(
            ...
            'mysql_version' => array(
                'label' => 'MySQL Version',
                'value' => (isset($wpdb) && method_exists($wpdb, 'db_version')) ? $wpdb->db_version() : 'Unknown',
                'status' => 'ok',
            ),
     ...
     */
    public function scan_file_for_errors($file_path, $lines = 100, $filter_plugin = false) {
