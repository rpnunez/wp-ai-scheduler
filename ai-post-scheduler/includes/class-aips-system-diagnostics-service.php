...
class AIPS_System_Diagnostics_Service {
    ...
    /**
     * Gets the gathered system information.
     *
     ...
     */
    /**
     * Counts instances of a specific cron hook.
     *
    ...
    private function count_cron_hook_instances( $hook ) {
        ...
        /**
     * Gets timestamps for a specific cron hook.
     *
     * @param string $hook The hook name.
     * @return array<int>
     */
    ...
    private function check_scheduler_health() {
        ...
        if ( $generation['schedule_success_rate'] >= 0 ) {
            ...
            $schedule_status = $schedule_rate >= AIPS_System_Status::METRIC_OK_THRESHOLD ? 'ok' : ( $schedule_rate >= AIPS_System_Status::METRIC_WARN_THRESHOLD ? 'warning' : 'error' );
    ...
    private function check_queue_health() {
        ...
        $stuck_status = 'ok';
        if ( $stuck >= AIPS_System_Status::QUEUE_STUCK_ERROR_THRESHOLD ) {
            $stuck_status = 'error';
        } elseif ( $stuck >= AIPS_System_Status::QUEUE_STUCK_WARN_THRESHOLD ) {
        ...
        if ( $qh['retry_saturation_pct'] >= 0 ) {
            ...
            $sat_status = 'ok';
            if ( $sat > AIPS_System_Status::QUEUE_RETRY_ERROR_THRESHOLD ) {
                $sat_status = 'error';
            } elseif ( $sat >= AIPS_System_Status::QUEUE_RETRY_WARN_THRESHOLD ) {
                $sat_status = 'warning';
            }
    ...
    private function check_generation_metrics() {
        ...
        $success_status = $m['success_rate'] >= AIPS_System_Status::METRIC_OK_THRESHOLD ? 'ok' : ( $m['success_rate'] >= AIPS_System_Status::METRIC_WARN_THRESHOLD ? 'warning' : 'error' );
        ...
        if ( $m['image_failure_rate'] >= 0 ) {
            $img_status = $m['image_failure_rate'] <= AIPS_System_Status::IMAGE_FAIL_OK_THRESHOLD ? 'ok' : ( $m['image_failure_rate'] <= AIPS_System_Status::IMAGE_FAIL_WARN_THRESHOLD ? 'warning' : 'error' );
     ...
     */
    /**
     * Checks the resilience settings.
     *
     * @return array<string, mixed>
     */
    ...
    /**
     * Checks the database health and structure.
     *
     * @return array<string, mixed>
     */
    ...
    /**
     * Checks WordPress cron configuration.
     *
     * @return array<string, mixed>
     */
    ...
    private function check_logs() {
        ...
        /**
     * Scans a file for errors.
     *
     * @param string $file_path The path to the file.
     * @param int $lines Number of lines to scan.
     * @param bool $filter_plugin Whether to filter by plugin name.
     * @return array<string>
     */
