...
class AIPS_System_Status {
    ...
    private function check_database() {
        ...
        // Use the centralized schema definition from AIPS_DB_Manager
        $db_manager = new AIPS_DB_Manager();
        $tables = $db_manager->get_schema_definitions();

        // Fallback just in case parsing fails (though unlikely if get_schema is correct)
        if (empty($tables)) {
             $tables = array(
                'aips_history' => array(
