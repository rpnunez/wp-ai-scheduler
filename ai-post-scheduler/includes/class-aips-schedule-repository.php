...
class AIPS_Schedule_Repository {
    ...
    /**
     * Get schedules due to run, ensuring the associated template is also active.
     *
     ...
     */
    public function get_due_schedules_with_active_templates($limit = 5, $current_time = null) {
        if ($current_time === null) {
            $current_time = current_time('mysql');
        }

        // Resolving column collisions by selecting s.* last or aliasing appropriately
        // The original query selected t.*, s.*, s.id as schedule_id.
        return $this->wpdb->get_results($this->wpdb->prepare("
