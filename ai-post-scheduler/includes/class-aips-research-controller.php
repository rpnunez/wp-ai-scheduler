...
class AIPS_Research_Controller {
    ...
    public function ajax_research_topics() {
        ...
        if (isset($_POST['keywords'])) {
            $unslashed_keywords = wp_unslash($_POST['keywords']);
            if (is_array($unslashed_keywords)) {
                foreach ($unslashed_keywords as $kw) {
                    if (is_scalar($kw)) {
                        $keywords[] = sanitize_text_field((string) $kw);
                    }
                }
            } elseif (is_scalar($unslashed_keywords)) {
                $keywords[] = sanitize_text_field((string) $unslashed_keywords);
            }
        }
