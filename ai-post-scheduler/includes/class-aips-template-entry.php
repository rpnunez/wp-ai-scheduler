...
class AIPS_Template_Entry {
	...
	public static function from_template_and_overrides(
		...
		return new self(
			...
			(string) ($source->featured_image_source ?? 'ai_prompt'),
			...
			isset($source->post_category) && $source->post_category !== null ? (int) $source->post_category : null,
			isset($source->post_tags) && $source->post_tags !== '' ? (string) $source->post_tags : null,
			isset($source->post_author) && $source->post_author !== null ? (int) $source->post_author : null,
			$post_quantity,
			$article_structure_id,
