...
class Test_AIPS_Upgrades extends WP_UnitTestCase {
	...
	/**
	 * check_and_run() with a pre-2.3.0 version must add the composite indexes
	 * to aips_notifications via the versioned migrate_to_2_3_0() migration.
	 */
	public function test_migrate_to_2_3_0_adds_composite_indexes() {
		// Simulate an existing install at a pre-2.3.0 version that has the
		...
		// Drop the composite indexes to simulate the pre-2.3.0 state.
		...
		// Trigger upgrade from a version older than 2.3.0.
		update_option( 'aips_db_version', '2.2.0' );
		...
		$this->assertContains(
			'is_read_created_at',
			$index_names_after,
			'migrate_to_2_3_0 must add is_read_created_at index'
		);
		$this->assertContains(
			'dedupe_key_created_at',
			$index_names_after,
			'migrate_to_2_3_0 must add dedupe_key_created_at index'
		);
	}
