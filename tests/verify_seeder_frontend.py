...
def verify_seeder_frontend():
    with sync_playwright() as p:
        ...
        def handle_route(route):
            ...
            elif "type=templates" in post_data:
                route.fulfill(status=200, body='{"success": true, "data": {"message": "Created 2 templates"}}', headers={'Content-Type': 'application/json'})
            else:
                route.fulfill(status=400, body='{"success": false, "data": {"message": "Unknown type"}}', headers={'Content-Type': 'application/json'})

        # Intercept the request. Note that in jQuery AJAX, the URL might be just '/mock-ajax' relative to the file,
        # but Playwright sees file:///mock-ajax which is invalid, or it resolves relative to file path.
        # Since we set window.aipsAjax.ajaxUrl = '/mock-ajax', jQuery tries to POST to file:///mock-ajax (roughly).
        # We'll match broadly.
        ...
        # Wait for specific log text with timeout
        try:
            page.wait_for_selector("#aips-seeder-log div:has-text('Created 5 voices')", timeout=5000)
            page.wait_for_selector("#aips-seeder-log div:has-text('Created 2 templates')", timeout=5000)
        ...
        # Take screenshot
        screenshot_path = f"{cwd}/verification_seeder.png"
