...
- [x] **Registry** — added 5 new types to `get_notification_type_registry()` with `default_mode = 'db'`
- [x] **Convenience methods** — added `template_generated()`, `manual_generation_completed()`, `post_ready_for_review()`, `post_rejected()`, `partial_generation_completed()` to `AIPS_Notifications`
- [x] **Hook bindings** — added bindings for `aips_schedule_execution_completed`, `aips_post_generated`, `aips_post_review_deleted`, `aips_post_generation_incomplete`
- [x] **Email templates** — added info/warning templates for all 5 types in `AIPS_Notification_Templates`
- [x] **Settings UI** — high-priority settings now include High-tier types via `get_high_priority_notification_types()`
- [x] **Config defaults** — added 5 new defaults to `aips_notification_preferences` in `AIPS_Config` with `'db'`
- [x] **Sanitizer** — updated `sanitize_notification_preferences()` fallback behavior for new notification types
- [x] **Emitters**:
...
### Outstanding for High tier

- [ ] Add/adjust PHPUnit coverage for new High-tier types and hook handlers
- [ ] Run full notification-related test suite after test updates
