...
class FeatureScanner:
    ...
    def generate_mermaid_flowchart(self, category: str, classes: List[str]) -> str:
        ...
        for class_name in classes:
            ...
            node_id = class_name.replace('AIPS_', '').replace('_', '')
    ...
    def generate_report(self, output_file: str):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            # Header
            f.write("# AI Post Scheduler - Feature Documentation\n\n")
            ...
            f.write("2. [Architecture Diagram](#architecture-diagram)\n")
            f.write("3. [Feature Categories](#feature-categories)\n")
            for category in categories.keys():
                anchor = category.lower().replace(' ', '-').replace('&', 'and')
                f.write(f"   - [{category}](#{anchor})\n")
            f.write("4. [Feature Profiles](#feature-profiles)\n")
            f.write("5. [Summary Statistics](#summary-statistics)\n\n")
            
            # Overview
            f.write("## Overview\n\n")
            f.write(f"This document provides comprehensive documentation for the AI Post Scheduler WordPress plugin. ")
            f.write(f"The plugin consists of **{len(self.features)} core classes** organized into ")
            f.write(f"**{len(categories)} functional categories**.\n\n")
            ...
            f.write("### Overall Plugin Architecture\n\n")
            f.write(self.generate_detailed_architecture_diagram())
            f.write("\n")
            ...
            for category, classes in categories.items():
                f.write(f"### {category}\n\n")
                f.write(f"This category contains {len(classes)} classes:\n\n")
                
                for class_name in classes:
                    feature = self.features[class_name]
                    f.write(f"- **{feature['name']}** (`{class_name}`): {feature['summary']}\n")
                
                f.write(f"\n#### {category} Architecture\n\n")
                f.write(self.generate_mermaid_flowchart(category, classes))
                f.write("\n")
            
            # Feature Profiles
            f.write("## Feature Profiles\n\n")
            f.write("Detailed analysis of each feature including files, functionality, and recommendations.\n\n")
            ...
            for class_name in sorted(self.features.keys()):
                ...
                f.write(f"**Summary**: {feature['summary']}\n\n")
                
                # 3. Files Involved
                f.write(f"**File**: `ai-post-scheduler/includes/{feature['file']}`\n\n")
                f.write(f"**Class**: `{class_name}`\n\n")
                f.write(f"**Lines of Code**: {feature['lines_of_code']}\n\n")
                
                # Technical Details
                f.write("**Technical Details**:\n\n")
                
                if feature['methods']:
                    f.write(f"- **Public Methods** ({len(feature['methods'])}): ")
                    f.write(", ".join(f"`{m}()`" for m in feature['methods'][:10]))
                    if len(feature['methods']) > 10:
                        f.write(f", ... and {len(feature['methods']) - 10} more")
                    f.write("\n")
                
                if feature['dependencies']:
                    f.write(f"- **Dependencies** ({len(feature['dependencies'])}): ")
                    f.write(", ".join(f"`{d}`" for d in feature['dependencies']))
                    f.write("\n")
                
                if feature['hooks']['actions']:
                    f.write(f"- **Action Hooks** ({len(feature['hooks']['actions'])}): ")
                    f.write(", ".join(f"`{h}`" for h in set(feature['hooks']['actions'][:5])))
                    if len(feature['hooks']['actions']) > 5:
                        f.write(f", ... and {len(feature['hooks']['actions']) - 5} more")
                    f.write("\n")
                
                if feature['hooks']['filters']:
                    f.write(f"- **Filter Hooks** ({len(feature['hooks']['filters'])}): ")
                    f.write(", ".join(f"`{h}`" for h in set(feature['hooks']['filters'][:5])))
                    if len(feature['hooks']['filters']) > 5:
                        f.write(f", ... and {len(feature['hooks']['filters']) - 5} more")
                    f.write("\n")
                
                if feature['ajax_handlers']:
                    f.write(f"- **AJAX Handlers**: ")
                    f.write(", ".join(f"`wp_ajax_{h}`" for h in feature['ajax_handlers']))
                    f.write("\n")
                ...
                if db_features:
                    f.write(f"- **Database Operations**: {', '.join(db_features)}\n")
                
                # WordPress API usage
                wp_api = [k.replace('uses_', '').replace('_', ' ').title() 
                         for k, v in feature['wp_api_usage'].items() if v]
                if wp_api:
                    f.write(f"- **WordPress APIs Used**: {', '.join(wp_api)}\n")
                ...
                if missing:
                    ...
                    f.write("**Missing Functionality**: None identified\n\n")
            ...
            # Classes by category
            f.write("### Classes by Category\n\n")
            f.write("| Category | Count | Classes |\n")
            f.write("|----------|-------|----------|\n")
            for category, classes in categories.items():
                class_list = ", ".join([c.replace('AIPS_', '') for c in classes[:3]])
                if len(classes) > 3:
                    class_list += f", ... ({len(classes) - 3} more)"
                f.write(f"| {category} | {len(classes)} | {class_list} |\n")
            
            f.write("\n")
            
            # Top classes by lines of code
            f.write("### Largest Classes (by Lines of Code)\n\n")
            f.write("| Class | Lines | File |\n")
            f.write("|-------|-------|------|\n")
            sorted_by_loc = sorted(self.features.items(), 
                                  key=lambda x: x[1]['lines_of_code'], 
                                  reverse=True)[:10]
            for class_name, feature in sorted_by_loc:
                f.write(f"| {feature['name']} | {feature['lines_of_code']} | `{feature['file']}` |\n")
            
            f.write("\n")
            
            # Most connected classes
            f.write("### Most Connected Classes (by Dependencies)\n\n")
            f.write("| Class | Dependencies | Depends On |\n")
            f.write("|-------|--------------|------------|\n")
            ...
            for class_name, feature in sorted_by_deps:
                ...
                if dep_count > 0:
                    ...
                    f.write(f"| {feature['name']} | {dep_count} | {deps} |\n")
            
            f.write("\n")
            
            # Footer
            f.write("---\n\n")
            f.write("*This report was automatically generated by the Feature Scanner tool.*\n")
