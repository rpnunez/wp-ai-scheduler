...
class AIPS_DevStackTips_Setup {
	...
	private function configure_settings() {
		...
		foreach ($settings['content_strategy'] as $key => $value) {
			update_option($key, $value);
			echo "[OK] Set {$key}\n";
		}

		echo "<h3>Resilience & Limits (Production Settings)</h3>\n";
		foreach ($settings['resilience_settings'] as $key => $value) {
			update_option($key, $value);
		...
		if (isset($structures['Evergreen How-To Guide'])) {
			...
			echo "[OK] Set default article structure: Evergreen How-To Guide (ID: {$structures['Evergreen How-To Guide']})\n";
		}

		echo "<h3>Notification Preferences</h3>\n";
		update_option('aips_review_notifications_email', '');
		echo "[WARN] Email not configured - Please add notification email in Settings > Notifications\n";
		update_option('aips_notification_preferences', $settings['notification_preferences']);
		echo "[OK] Set notification preferences (email for critical, DB for routine)\n";

		echo "<h3>Research & Discovery</h3>\n";
		update_option('aips_research_niches', $settings['research_niches']);
		echo "[OK] Set research niches (8 DevStackTips topics)\n";
		update_option('aips_topic_similarity_threshold', $settings['topic_similarity_threshold']);
		echo "[OK] Set topic similarity threshold: {$settings['topic_similarity_threshold']} (allows more variety)\n";

		echo "<h3>Featured Images</h3>\n";
		update_option('aips_unsplash_access_key', $settings['unsplash_access_key']);
		echo "[WARN] Unsplash key not configured - Please add Unsplash Access Key in Settings if using Unsplash images\n";

		echo "<h3>Performance Monitoring</h3>\n";
		update_option('aips_enable_telemetry', $settings['enable_telemetry']);
		echo "[OK] Enabled telemetry (slow query & request tracking)\n";

		echo "<h3>Cache Configuration</h3>\n";
		update_option('aips_enable_cache_system', $settings['enable_cache_system']);
		echo "[OK] Disabled cache system\n";

		echo "<h3>Log Management</h3>\n";
		update_option('aips_log_retention_days', $settings['log_retention_days']);
		echo "[OK] Set log retention: {$settings['log_retention_days']} days\n";

		echo "<h3>Default Post Settings</h3>\n";
		update_option('aips_default_post_status', $settings['default_post_status']);
		echo "[OK] Set default post status: {$settings['default_post_status']}\n";

		$categories = $this->get_created_categories_by_name();
		if (isset($categories[$settings['default_category_name']])) {
			update_option('aips_default_category', $categories[$settings['default_category_name']]);
			echo "[OK] Set default category: {$settings['default_category_name']} (ID: {$categories[$settings['default_category_name']]})\n";
		}

		update_option('aips_default_post_author', $settings['default_post_author']);
		echo "[OK] Set default post author: {$settings['default_post_author']} (admin)\n";

		echo "[OK] All plugin settings configured for production\n";
	}
	...
	/**
	 * Configures plugin settings for content strategy, resilience, notifications, and defaults.
	 *
	 * Sets up content strategy options, resilience and rate limiting settings,
	 * default article structure, and notification preferences for the DevStackTips plugin.
	 *
	 * @return void
	 */
	private function configure_settings_legacy() {
		echo "<h2>Step 4: Configuring Plugin Settings</h2>\n";
		$settings = $this->object_data['settings'];
		...
		echo "[OK] Set topic similarity threshold: 0.75 (allows more variety)\n";
		...
		// Cache System
		echo "<h3>Cache Configuration</h3>\n";
		update_option('aips_enable_cache_system', false);
		echo "[OK] Disabled cache system\n";

		// Log Retention
		echo "<h3>Log Management</h3>\n";
		update_option('aips_log_retention_days', 60);
		echo "[OK] Set log retention: 60 days\n";

		// Default Post Settings
		echo "<h3>Default Post Settings</h3>\n";
		update_option('aips_default_post_status', 'draft');
		echo "[OK] Set default post status: draft\n";
		
		$categories = $this->get_created_categories_by_name();
		if (isset($categories['Backend Development'])) {
			update_option('aips_default_category', $categories['Backend Development']);
			echo "[OK] Set default category: Backend Development (ID: {$categories['Backend Development']})\n";
		}
		
		update_option('aips_default_post_author', 1);
		echo "[OK] Set default post author: 1 (admin)\n";

		echo "[OK] All plugin settings configured for production\n";
	}

	private function create_categories_legacy() {
		echo "<h2>Step 1: Creating Categories</h2>\n";
	...
	private function create_voices_legacy() {
		echo "<h2>Step 3: Creating Voices</h2>\n";
...
- Responsible AI practices',
	...
	private function create_article_structures_legacy() {
		echo "<h2>Step 4: Creating Article Structures</h2>\n";
	...
	private function create_authors_legacy() {
		echo "<h2>Step 5: Creating Authors</h2>\n";
	...
	private function create_post_slices_legacy() {
		echo "<h2>Step 6: Creating Post Slices</h2>\n";
	...
	private function create_source_groups_legacy() {
		echo "<h2>Step 7: Creating Source Groups and Sources</h2>\n";
	...
	private function create_templates_legacy() {
		echo "<h2>Step 9: Creating Templates</h2>\n";
	...
	private function create_campaigns_legacy() {
		echo "<h2>Step 8: Creating Campaigns</h2>\n";
	...
	private function create_schedules_legacy() {
		echo "<h2>Step 10: Creating Schedules</h2>\n";

		$schedule_repo = new AIPS_Schedule_Repository();
		$templates = $this->get_created_templates_by_name();

		// Deterministic schedule matrix targeting 30 posts/week via template post_quantity:
		// - Core weekday tracks: Mon-Fri (20 posts/week total)
		// - Flex tracks: Tue/Thu/Sat (10 posts/week total)
		$schedules = array(
			array(
	...
	private function get_settings_seed_data() {
		return array(
			...
			'resilience_settings' => array(
				'aips_enable_retry' => 1,
				'aips_retry_max_attempts' => 3,
				'aips_retry_initial_delay' => 2,
				'aips_enable_rate_limiting' => 1,
				'aips_rate_limit_requests' => 20,
			...
			'default_category_name' => 'Backend Development',
		);
	}
...
- Responsible AI practices',
	...
	private function print_summary() {
		...
		echo "<li>Verify Schedules are configured correctly (30 posts/week target)</li>\n";
	...
	private function rollback() {
		...
		$schedule_titles = array(
			'Core Monday - Developer Foundations',
			'Core Tuesday - Backend Engineering',
			'Core Wednesday - Security First',
			'Core Thursday - Framework Comparisons',
			'Core Friday - Developer Tooling',
			'Flex Tuesday PM - AI Workflow Insights',
			'Flex Thursday PM - Security Intelligence Briefing',
			'Flex Saturday - PHP Ecosystem Radar',
		);
