...
2. **Controller responsibilities**
   ...
   - Handle nonce checks (pass `false` as the third arg to `check_ajax_referer`), capability checks, sanitization, and response formatting.
   - Return `AIPS_Ajax_Response::error()` on nonce/capability failure — never rely on `wp_die()`.
...
## Useful agents
- `.claude/agents/ajax-controller-reviewer.md` — review pass after implementation
