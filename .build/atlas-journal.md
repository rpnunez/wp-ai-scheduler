...
## 2026-01-06 - Add Advanced Rule-Based Scheduling
**Context:** Scheduling was limited to fixed intervals, preventing users from expressing richer timing needs (e.g., “between 8–10 AM on Mondays and Wednesdays, except the 15th”). This coupled interval math with complex calendar logic inside the scheduler and UI, limiting extensibility.
**Decision:** Introduced a rule-based scheduling layer. Added `AIPS_Advanced_Schedule_Evaluator` to encapsulate AND/OR rule evaluation and next-run calculations, extended the schedule schema with `schedule_type` and `rules`, and updated the scheduler/controller/UI to support a JS-managed builder for advanced conditions while preserving the interval flow.
**Consequence:** 
* Pros: Advanced schedules allow combinable time/day constraints without disrupting interval-based schedules. Logic is isolated in a dedicated evaluator for cohesion and reuse. UI exposes a structured builder while keeping interval mode unchanged. Database changes are additive with safe defaults.
* Cons/Trade-offs: Additional columns and evaluator add complexity and minor storage overhead. Next-run calculation for rules iterates future minutes (bounded) which is acceptable for admin-driven saves.
* Backward Compatibility: Default `schedule_type` is `interval`; existing schedules retain behavior. Migration adds columns with defaults; interval validation still enforced. Advanced schedules compute next runs independently to avoid altering interval math.
**Tests:** Added `tests/test-advanced-schedule-evaluator.php` covering rule sanitization, matching, exclusions, and next-run calculation (not executed in this environment).
