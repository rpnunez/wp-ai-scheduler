...
## 2026-03-08 - Extract Admin Menu Logic from AIPS_Settings God Object
**Context:** The `AIPS_Settings` class was a classic "God Object" responsible for handling plugin settings, fields registration, admin menu registration, and the rendering logic for nearly all admin pages. This violated the Single Responsibility Principle and made the class difficult to navigate, test, and maintain.
**Decision:** Extracted the admin menu registration (`add_menu_pages`, `fix_author_topics_parent_file`, `fix_author_topics_submenu_file`) and rendering callbacks into a new, dedicated `AIPS_Admin_Menu` class. The `render_page` method was also added to individual page controllers (e.g. `AIPS_Schedule_Controller`, `AIPS_Authors_Controller`, `AIPS_Structures_Controller`), shifting the responsibility of page rendering from `AIPS_Settings` to the respective controllers. Deprecated Activity AJAX endpoints and rendering hooks were fully removed as well.
**Consequence:** The `AIPS_Settings` class is significantly smaller and solely responsible for plugin settings configuration. The new `AIPS_Admin_Menu` class cleanly routes submenu pages to the right controllers. Loose coupling is enforced since the menu class relies on individual controllers for rendering instead of handling it directly.
**Tests:** Verified backward compatibility by confirming the submenu hooks, menu generation, and the instantiation of `AIPS_Admin_Menu` in the core plugin file. Tests were run, though many failed locally due to missing `wordpress-tests-lib` constraints.
