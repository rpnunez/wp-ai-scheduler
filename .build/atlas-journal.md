...
## 2026-08-31 - [Refactor AIPS_DB_Manager God Method]
**Context:** AIPS_DB_Manager::get_schema() was a massive God method violating the Single Responsibility Principle by defining over 30 tables in one function.
**Decision:** Applied "Separation of Concerns" by extracting table creation SQL statements into separate, domain-specific private methods (e.g., get_history_schema, get_campaign_schema, etc.) and aggregating their return values in get_schema.
**Consequence:** Vastly improved readability and maintainability of the database manager. No external API changes were introduced, strictly preserving backwards compatibility.
**Tests:** Ran syntax checks (php -l), local sanity checks (test_tables.php), and the PHPUnit test suite to ensure all tables are still correctly generated by get_schema().
