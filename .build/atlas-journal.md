...
## 2026-03-08 - [Extract Internationalization Strings from Assets]
**Context:** The `AIPS_Admin_Assets` class was heavily bloated with large arrays of localized translation strings (`wp_localize_script`) for every JavaScript module in the admin dashboard. This coupled string definitions and translation boundaries directly with asset enqueueing, violating the Single Responsibility Principle and making the assets class extremely difficult to read and maintain. Furthermore, there was no `.pot` file present to officially support WordPress Polyglots translation processes.
**Decision:** Extracted all translation strings into a dedicated, static helper class: `AIPS_i18n`. This class serves purely as a repository for translatable strings (e.g., `AIPS_i18n::get_authors_strings()`), keeping the logic of "what the string is" separate from "how it is delivered to the browser" (which remains in `AIPS_Admin_Assets`). Additionally, generated the official `ai-post-scheduler.pot` file in the `languages/` directory using WP-CLI `wp i18n make-pot`, fully implementing the WordPress Gettext standard.
**Consequence:** `AIPS_Admin_Assets` is now strictly focused on conditionally enqueueing files and passing configuration data (like nonces and URLs), drastically reducing its size and improving readability. Translators now have a central `.pot` file to generate `.po` and `.mo` files without searching through JS or PHP files. The refactoring was done carefully to maintain backwards compatibility with existing JavaScript files that expect these objects (like `aipsAdminL10n`).
**Tests:** Created `Test_I18n.php` to verify that all translation string arrays are correctly formed and returned by the new `AIPS_i18n` class.
