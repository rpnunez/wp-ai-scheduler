...
**Context:** The `AIPS_Settings` class was functioning as a God Object. It handled plugin settings registration, various unrelated AJAX requests, and essentially all WordPress admin menu registration and template rendering. This violated Separation of Concerns, making `AIPS_Settings` unwieldy and coupled to too many features.
**Decision:** Extracted the admin menu registration (`add_menu_pages`) and all UI rendering methods (`render_*_page`) out of `AIPS_Settings` into a new, dedicated `AIPS_Admin_Menu` class.
**Consequence:** `AIPS_Settings` is now solely responsible for plugin settings registration (`register_settings`) and settings-related AJAX tasks. `AIPS_Admin_Menu` now centrally handles the `admin_menu` hook and routing to various controller render methods. This creates higher cohesion within `AIPS_Settings` and isolates menu/routing logic, improving overall maintainability.
**Tests:** Created `tests/test-aips-admin-menu.php` which specifically verifies that `AIPS_Admin_Menu` successfully registers the parent menu and all expected submenu pages. Executed full test suite to ensure no regressions in backward compatibility.
