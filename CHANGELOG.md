...
### Added
- **Content Refresher Settings**: The Autonomous Post Refresher is now fully configurable under Settings > Content Refresher — enable/disable, scan frequency, staleness threshold (was a hard-coded 180 days), posts per scan, post types, auto-approve, default immutability, and audit-log retention. A `aips_post_refresher_scan` WP-Cron event now actually runs the scan on the configured schedule; previously nothing ran it automatically.
- **Immutable Posts**: An "AI Content Refresher" meta box on the post editor lets an admin mark a post Immutable so the refresher never proposes or applies an update to it. Immutable posts are excluded at query level and rejected if targeted directly. Exposed over AJAX as `aips_set_post_immutable`.
- **Per-Post Audit Log**: Every refresher check is appended to the `_aips_audit_log` post meta (result code, timestamp, message, revision ID), including skips, with `_aips_last_audited_at` and `_aips_audit_check_count` companions. Readable in the editor meta box and over AJAX as `aips_get_post_audit_log`. The log is trimmed to the configured entry limit.
...
### Fixed
- **Refresher Content Escaping**: `wp_update_post()`/`wp_insert_post()` unslash their input, so approving a revision (and persisting a generated draft) stripped backslashes from content — corrupting file paths, regex, and LaTeX on every refresh. All three call sites now pass `wp_slash()`ed data.
- **Stuck Pending Flags**: `_aips_has_pending_revision` was only cleared by approve/reject, so a staging draft deleted any other way excluded its live post from every future scan with no way to clear it. Scans now reconcile dangling flags, and trashing or deleting a staging draft releases its target immediately.
- **Unreachable Staged Revisions**: staged revisions had no admin surface at all — the drafts are hidden from the post list and nothing called the review endpoints. Live posts with a pending refresh now expose Review / Approve / Discard row actions.
- **Refresher Reschedule Timing**: `update_option_{$option}` fires before the option cache is invalidated, so changing the enabled flag and frequency in one save rescheduled the cron with the stale frequency. The reschedule is now deferred to `shutdown` and re-reads uncached values.
- **Immutable Post-State Labels**: with "Protect Posts By Default" enabled, every row in the post list was badged "Immutable". Labels now reflect the explicitly stored value, and mark deliberately opted-in posts as "Refreshable".
- **Post Refresher Generation**: `AIPS_Post_Audit_Service` built its context with `new AIPS_Topic_Context(array(...))`, but that class takes an author and topic object — every refresh attempt raised an `ArgumentCountError`. Replaced with a dedicated `AIPS_Post_Refresh_Context` implementing `AIPS_Generation_Context`.
- **Duplicate Staging Drafts**: The refresher inserted its own draft on top of the one the pipeline's review-preparation stage had already persisted, leaving an orphan draft per scanned post. The pipeline's draft is now reused, and a partial draft from a failed pipeline run is cleaned up.
- **Destructive Revision Endpoints**: `approve_revision()` and `reject_revision()` accepted any post ID and force-deleted it (and, for approve, overwrote an unrelated post with its content). Both now require the target to carry `_aips_is_staging_revision`.
- **Refresher Staging Drafts in Post List**: Staging revisions are ordinary drafts, so they appeared alongside real drafts and invited accidental publishing. They are now hidden from the admin post list and labelled with post states where shown.
- **Post Audit AJAX Responses**: HTTP status codes were being passed in the error-code argument, and the handlers checked a nonce action (`aips_admin_nonce`) that the plugin never issues. Both corrected; `limit` is now clamped to the configured ceiling instead of a hard-coded 20.
