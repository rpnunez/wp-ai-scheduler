...
- **Embeddings Repository Caching**: `AIPS_Embeddings_Repository` caches single-object lookups, batched post lookups, counts, index statistics, and stored dimensions under the broad `embeddings` tag bumped by every write. The `wp_posts`-joined `count_indexed_for_types()` additionally carries an `embeddings_posts` tag that the new `AIPS_Post_Lifecycle_Cache_Invalidator` bumps on `transition_post_status` / `deleted_post`, so trashing, publishing, or deleting posts through native WordPress flows never serves stale index counts. Full-vector similarity reads and the backfill queue cursor stay uncached.
- **Relationships, Integration Mappings, and Content Audit Caching**: `AIPS_Relationships_Repository`, `AIPS_Integration_Mappings_Repository`, and `AIPS_Content_Auditor_Repository` now adopt both repository traits. Related-post lookups, duplicate pairs, graph interconnections, per-template field mappings, and audit history/counts are cached under broad tags bumped by every write (bulk syncs invalidate once). `wp_posts`-joined relationship reads carry a `relationships_posts` tag that the post lifecycle invalidator also bumps; the invalidator now resolves its target repositories lazily instead of constructing them on every request.
- **Central Cache Dependency Map**: `AIPS_Repository_Cache_Dependencies` is now the single source of truth for repository cache tags. `READ_TAGS` declares every cached operation's tags, `INVALIDATION_TAGS` declares every write domain, and repositories no longer declare inline policy `tags` or hand-built invalidation tag lists — every write goes through `invalidate_cache_domain()`.
- **Cross-Repository Cache Invalidation**: Tag versions are scoped per cache group, so the map's new `DEPENDENTS` table declares which other repositories' joined reads each write domain affects, and `invalidate_cache_domain()` fans the bump out to them. The dashboard and metrics caches now refresh on history, author-topic, author, schedule, and template writes (dashboard reads carry precise per-table tags); generated-post topic logs refresh author-topic status counts; topic deletions refresh feedback and topic-log counts; and schedule changes refresh per-schedule history counts.
- **Cache Correctness Fixes**: Repository `create()` / `insert()` methods now capture `$wpdb->insert_id` before cache invalidation, whose bookkeeping writes were overwriting it and making creates return the wrong row ID. Repositories re-resolve their invalidation cache instances after `AIPS_Cache_Factory::reset()`. The History list renders raw (unprepared) rows and rows with unparseable dates without undefined-property errors. Data imports now flush every named repository cache instance via `AIPS_Cache_Factory::flush_all()` (not just the shared instance), clearing history by status now invalidates cached history stats, the content-audit repository honors an injected `wpdb`, and saving a content audit no longer fatals on the non-existent `AIPS_DateTime::now_ts()`.
