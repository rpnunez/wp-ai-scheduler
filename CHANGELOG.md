...
- **Cross-Repository Cache Invalidation**: Tag versions are scoped per cache group, so repositories whose writes change another repository's joined reads now delegate the bump through `AIPS_Cacheable_Repository::invalidate_repository_cache_tags()`. Generated-post topic logs refresh author-topic status counts, and topic deletions refresh feedback and topic-log counts.
- **Cache Correctness Fixes**: Data imports now flush every named repository cache instance via `AIPS_Cache_Factory::flush_all()` (not just the shared instance), clearing history by status now invalidates cached history stats, the content-audit repository honors an injected `wpdb`, and saving a content audit no longer fatals on the non-existent `AIPS_DateTime::now_ts()`.
