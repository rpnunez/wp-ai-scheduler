...
- [2026-01-17] Added "View All Schedules" link to the Upcoming Scheduled Posts dashboard widget for better navigation.
...
- [2026-01-17] Fixed potential race condition in schedule processing by implementing optimistic locking (compare-and-swap) when updating `next_run` timestamps.

### Architecture
- [2026-01-17] Refactored Article Structure UI handlers from `admin.js` into a dedicated `admin-structures.js` module for better maintainability and separation of concerns.
...
- [2026-01-17] Optimized `AIPS_Template_Type_Selector` to eliminate N+1 subqueries when counting schedule executions by utilizing pre-fetched data.
