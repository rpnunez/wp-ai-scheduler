...
### Fixed
- **Sanitize HTML in AI Prompts**: Convert block-level HTML tags (such as `<p>`, `<h2>`, `<br>`) into line breaks and strip all HTML tags and decode entities from Post Content before embedding it into AI prompts for Title, Excerpt, Metadata, Taxonomy, and Link Inserter requests, reducing token consumption and preventing HTML tag noise.
